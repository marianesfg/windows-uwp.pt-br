...
O Portal de Dispositivos Xbox fornece um conjunto padrão de páginas semelhante ao que está disponível no Portal de Dispositivos do Windows, bem como várias páginas que são exclusivas. Para obter descrições detalhadas delas, consulte [Visão geral do Portal de Dispositivos do Windows](../debug-test-perf/device-portal.md). As seções a seguir descrevem as páginas que são exclusivas do Portal de Dispositivos do Xbox.
