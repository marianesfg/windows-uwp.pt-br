...
Para adicionar um evento de pós-compilação personalizado, no Gerenciador de soluções, abra o menu de atalho para o projeto ToasterComponent e, em seguida, escolha Propriedades. No painel esquerdo, as páginas de propriedade, selecione compilar eventos e, em seguida, escolha o botão de pós-compilação editar. Adicione os seguintes comandos para a linha de comando de pós-compilação. (O arquivo em lotes deve ser chamado pela primeira vez para definir as variáveis de ambiente para encontrar a ferramenta winmdidl.)
...
Para garantir que o arquivo IDL é gerado novamente sempre que o arquivo. winmd é alterado, **executar o evento pós-compilação** para alterar **quando a compilação atualiza a saída do projeto.**
...
Por padrão, o projeto de Proxies gera arquivos. h do cabeçalho e C++. cpp. Como a DLL é compilada dos arquivos produzidos a partir MIDL, os arquivos. h e. cpp não são necessários. No Gerenciador de soluções, abra o menu de atalho para eles, escolha **Remover**e, em seguida, confirmar a exclusão.
...
-   O caminho é o caminho relativo do pacote do proxy binário. (Neste passo a passo, proxies.dll é na mesma pasta como ToasterApplication.winmd).
