---
author: stevewhims
description: O tempo de execução do Windows é um sistema de contagem de referência. e, nesse sistema é importante saber sobre o significado do e distinção entre, referências fracas e fortes.
...
Com essa alteração no código, executamos um problema na etapa 4, porque a instância de classe é destruída, e *Este* não é mais válida. Assim que o objeto assíncrono tenta acessar a variável dentro de instância da classe, ele será pane (ou fazer algo inteiramente indefinidas).
...
A listagem de código abaixo primeiro define uma classe **EventSource** simple, que dispara um evento genérico que é manipulado por qualquer delegados que foram adicionados a ela. Este evento de exemplo ocorre usar o tipo de representante [**Windows::Foundation::EventHandler**](/uwp/api/windows.foundation.eventhandler) , mas os problemas e soluções aqui se aplicam a tipos de delegado todo e qualquer.
...
> [!IMPORTANT]
> Se você encontrar uma situação assim, você precisará pensar sobre o tempo de vida *desse* objeto; e se o capturada *esse* objeto sobrevive a captura. Se isso não acontecer, então capture-o com uma referência forte ou fraca, como demonstraremos abaixo.
>
...
Acima, vimos referências fracas que está sendo usadas. Em geral, elas são ideais para quebrar referências cíclicas. Por exemplo, para a implementação nativa da estrutura da interface do usuário baseada em XAML&mdash;devido ao design da estrutura&mdash;mecanismo em C++ de referência fraca c++ WinRT é necessário para tratar referências cíclicas. Fora do XAML, no entanto, você provavelmente não precisará usar referências fracas (não há nada inerentemente XAML específicas sobre eles). Em vez disso, muito frequentemente, será capaz de criar seu próprio C + c++ APIs do WinRT de forma evitar a necessidade de referências cíclicas e referências fracas. 
