...
Comece criando um novo projeto no Microsoft Visual Studio. Criar um **Visual C++** > **Universal do Windows** > **componente do tempo de execução do Windows (C++ c++ WinRT)** projeto e nomeie- *BankAccountWRC* (para "conta bancária componente do tempo de execução do Windows").
...
## <a name="parameterized-delegates-and-simple-signals-across-an-abi"></a>Delegados com parâmetros e sinais simples, em uma ABI
...
Se você não precisará passar os parâmetros ou argumentos com seu evento, em seguida, você pode definir seu próprio tipo de representante de tempo de execução do Windows simples. O exemplo a seguir mostra uma versão mais simples da classe de tempo de execução **BankAccount** . Ele declara um tipo de representante chamado **SignalDelegate** e, em seguida, que ele usa para disparar um evento de tipo de sinal em vez de um evento com um parâmetro.
