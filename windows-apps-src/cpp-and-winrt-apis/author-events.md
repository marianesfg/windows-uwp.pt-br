...
O projeto recém-criado contém um arquivo chamado `Class.idl`. Renomeie o arquivo `BankAccount.idl` (renomear o `.idl` arquivo renomeia automaticamente o dependente `.h` e `.cpp` arquivos, muito). Substitua o conteúdo do `BankAccount.idl` na lista abaixo.
...
Os parâmetros de tipo para esses tipos de dois delegado têm cruzar a ABI, portanto, os parâmetros de tipo devem ser tipos do Windows Runtime, muito. Isso inclui classes de tempo de execução de primeiro e de terceiros, bem como tipos primitivos, como números e cadeias de caracteres. O compilador ajuda você com um erro "*deve ser tipo WinRT*" Se você esquecer essa restrição.
...
Se o evento é usado apenas internamente dentro do C++ c++ WinRT projeto (não em binários), então você usa o modelo de struct [**winrt::event**](/uwp/cpp-ref-for-winrt/event) , mas você parametrizá-lo com C++ c++ WinRT:: delegate [**de Windows Runtime do WinRT&lt;... T&gt; **](/uwp/cpp-ref-for-winrt/delegate) modelo de struct, que é um representante eficiente, contagem de referência. Ele dá suporte a qualquer número de parâmetros, e eles não são limitados aos tipos de tempo de execução do Windows.

O exemplo abaixo mostra primeiro um representante assinatura que não usa os parâmetros (essencialmente um sinal simple) e, em seguida, que aceita uma cadeia de caracteres.
...
Observe como você pode adicionar ao evento quantos representantes de assinatura conforme desejar. No entanto, há algumas sobrecarga associada a um evento. Se você só precisará de um retorno de chamada simple com apenas um único delegado da assinatura, você pode usar WinRT:: delegate [**&lt;... T&gt; **](/uwp/cpp-ref-for-winrt/delegate) por conta própria.
