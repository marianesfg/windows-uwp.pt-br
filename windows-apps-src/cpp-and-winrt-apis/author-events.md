...
Se quaisquer avisos impedirem construção, resolvê-los ou definir a propriedade do projeto **C/C++** > **Geral** > **Tratar avisos como erros** **não (/ /WX-)** e compile o projeto novamente.
...
Agora crie um novo projeto (em sua solução `BankAccountWRC` ou em uma nova). Criar um **Visual C++** > **Universal do Windows** > **aplicativo principal (C++ c++ WinRT)** projeto e nomeie-o *BankAccountCoreApp*.
...
É recomendável que você passe eventos e delegados não, como parâmetros de função. A função **Adicionar** [**winrt::event**](/uwp/cpp-ref-for-winrt/event) é a única exceção, porque você deve passar um delegado nesse caso. O motivo para essa diretriz é como delegados podem levar formas diferentes em diferentes idiomas de tempo de execução do Windows (em termos de se aceitam o registro de um cliente ou várias). Eventos, com o modelo de vários assinante, constituem uma opção muito mais previsível e consistente.
