...
Vamos criar uma nova classe para representar um controle personalizado de (modelo). Estamos criando e consumindo a classe dentro da mesma unidade de compilação. Mas queremos poder criar uma instância dessa classe do marcação XAML e por isso que ela será ser uma classe de tempo de execução. E vamos usar C++/WinRT para criar e consumi-lo.
...
Após a inclusão desses controles, seu `BgLabelControl.h` é semelhante a este.
...
Sob o nó do seu projeto, crie uma nova pasta e nomeie-a como "Themes". Em `Themes`, adicione um novo item do tipo **Visual C++** > **XAML** > **Modo de exibição XAML**e nomeie-a como "Generic". Os nomes de pasta e arquivo precisam ser assim para que a estrutura XAML encontrar o estilo padrão para um controle personalizado. Exclua o conteúdo padrão do `Generic.xaml`e cole na marcação abaixo.
