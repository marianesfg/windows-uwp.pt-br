...
> [!NOTE]
> A função **obter** existe em C++ c++ WinRT projeção digite **winrt::Windows::Foundation::IAsyncAction**, portanto, você pode chamar a função de dentro de qualquer C + c++ WinRT projeto. Você não encontrará a função listada como um membro da interface [**IAsyncAction**](/uwp/api/windows.foundation.iasyncaction) , porque não é parte da superfície de binários interface (ABI) do aplicativo do tipo real do Windows Runtime **IAsyncAction** **obter** .
...
Para esse cenário, há um pouco de ineffiency em torno da chamada para **StorageFile::OpenAsync**. Há uma atualização de contexto necessário para um plano de fundo thread (para que o manipulador pode retornar a execução para o chamador), na retomada após a qual C + c++ WinRT restaura o contexto do thread da interface do usuário. Mas, nesse caso, não é necessário ser no thread da interface do usuário até que estamos prestes a atualizar a interface do usuário. As APIs do tempo de execução do Windows mais que chamamos *antes de* nossa chamada para **WinRT:: resume_background**, as opções de contexto back bidirecional mais desnecessárias tivermos. A solução não é chamar *quaisquer* APIs do Windows Runtime antes. Movê-los todos após o **WinRT:: resume_background**.
...
Se você deseja fazer algo mais avançado, em seguida, você pode escrever seus próprios await adaptadores. Por exemplo, se você quiser um `co_await` para continuar no mesmo thread que a ação assíncrona seja concluída em (portanto, não há nenhum comutador de contexto), em seguida, você pode começar criando await adaptadores semelhantes àquelas mostrado abaixo.

> [!NOTE]
> O exemplo de código abaixo é fornecido apenas para fins educacionais; é para você começar Noções básicas sobre como await adaptadores trabalho. Se você quiser usar essa técnica em seu próprio código de base, em seguida, é recomendável que você desenvolver e testar seus próprios await struct(s) adaptador. Por exemplo, você pode escrever **complete_on_any**, **complete_on_current**e **complete_on(dispatcher)**. Além disso, considere torná-los modelos que usam o tipo de **IAsyncXxx** como um parâmetro de modelo.
...
Para entender como usar o **sem_mudança** await adaptadores, primeiro você precisará saber que, quando o compilador C++ encontra um `co_await` expressão, ele procura funções chamado **await_ready**, **await_suspend**e **await_resume**. C++ c++ WinRT biblioteca fornece essas funções para que você obter o comportamento do razoável por padrão, desta forma.
...
Em seguida, em vez de procurando as três funções **await_xxx** que correspondem ao **IAsyncXxx**, o compilador C++ procura funções que correspondem ao **sem_mudança**.

## <a name="canceling-an-asychronous-operation-and-cancellation-callbacks"></a>Cancelar uma operação assíncrona e os retornos de chamada de cancelamento

Recursos do Windows Runtime para programação assíncrona permitem que você cancelar uma operação ou ação assíncrona em trânsito. Aqui está um exemplo que chama [**StorageFolder::GetFilesAsync**](/uwp/api/windows.storage.storagefolder.getfilesasync) para recuperar uma coleção de arquivos potencialmente grande e armazena o objeto de operação assíncrona resultante em um membro de dados. O usuário tem a opção de cancelar a operação.
...
Se você executar o exemplo acima, verá **ImplicitCancellationAsync** impressão uma mensagem por segundo para três segundos, após o qual vez em que ele automaticamente encerra como resultado do cancelamento. Isso funciona porque, em encontrando um `co_await` expressão, uma rotina concomitante verifica se foi cancelada. Caso afirmativo, em seguida, ele reduz e, caso contrário, em seguida, suspende como normal.

Cancelamento pode, obviamente, acontecer enquanto a rotina concomitante é suspenso. Somente quando a corrotina é retomada, ou bate em outro `co_await`, ele verificará cancelamento. O problema é uma das potencialmente muito grande-mais refinado latência responder ao cancelamento.

Portanto, outra opção é explicitamente sondar cancelamento de dentro de sua rotina concomitante. Atualize o exemplo acima com o código na lista abaixo. Neste exemplo de novo, **ExplicitCancellationAsync** recupera o objeto retornado pela função [**winrt::get_cancellation_token**](/uwp/cpp-ref-for-winrt/get-cancellation-token) e utilizará para verificar periodicamente se a rotina concomitante tenha sido cancelada. Desde que não seja cancelado, a rotina concomitante permanece em loop indefinidamente; Depois que ele for cancelado, o loop e a função sair normalmente. O resultado é o mesmo, como o exemplo anterior, mas aqui saindo acontece explicitamente e sob o controle.
...
Neste exemplo de código próximo, **NestedCoroutineAsync** faz o trabalho, mas ele tem nenhuma lógica especial cancelamento nele. **CancellationPropagatorAsync** é essencialmente um wrapper sobre a rotina concomitante aninhado; o wrapper encaminha preventivamente cancelamento.
...
**CancellationPropagatorAsync** registra uma função lambda para seu próprio retorno de chamada de cancelamento e, em seguida, ele aguarda (ele suspende) até que o trabalho aninhado é concluída. Quando ou se **CancellationPropagatorAsync** for cancelado, ele propaga o cancelamento para a rotina concomitante aninhado. Não é necessário para pesquisar cancelamento; nem é cancelamento bloqueado indefinidamente. Esse mecanismo é flexível o suficiente para que você possa usá-lo para interoperabilidade com uma biblioteca de rotina concomitante ou simultaneidade que sabe nada do C++ c++ WinRT.
