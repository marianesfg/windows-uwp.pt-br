...
Em seguida, será possível criar e compilar ou abrir, C++ c++ WinRT projeto no Visual Studio e implantá-lo. Como alternativa, você pode converter um projeto existente adicionando a `<CppWinRTEnabled>true</CppWinRTEnabled>` propriedade para sua `.vcxproj` arquivo.
...
Porque C++ c++ /WinRT usa recursos a partir do padrão c++17, ele precisa da propriedade **C/C++** do projeto > **idioma** > **C++ idioma padrão** > **ISO padrão c++17 (/ std:c++17 + + 17)**. Talvez também seja necessário definir **Modo de conformidade: Sim (/permissive-)**, o que restringirá ainda mais seu código para que seja compatível com padrões.
