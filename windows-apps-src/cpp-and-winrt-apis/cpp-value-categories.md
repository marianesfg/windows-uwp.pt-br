...
ms.prod: windows
ms.technology: uwp
keywords: Windows 10, uwp, padrão, c++, cpp, winrt, projeção, mover, encaminhamento, categorias de valor, a semântica de movimento, encaminhamento perfeito, lvalue, rvalue, glvalue, prvalue, xvalue
...
Um *lvalue* tem identidade. Agora é uma questão de interesse apenas histórico que o "l" em "lvalue" é a abreviação de "esquerda" (como à esquerda-à lado de uma atribuição). Em C++, lvalue pode ser exibidos à esquerda *ou* à direita de uma atribuição. O "l" em "lvalues", em seguida, não realmente ajuda você a compreender nem definir quais eles são. Você só precisa compreender que o que chamamos de lvalue é um valor que tem identidade.
...
Mas há valores que não são glvalues. Consequentemente, há valores que você *não pode* obter um endereço de memória para (ou você não pode confiar para ser válido). Vimos alguns esses valores no exemplo de código acima. Isso parece uma desvantagem. Mas, na verdade a vantagem de um valor, como isto é que você pode *Mover* dele (que é geralmente baratos), em vez de cópia dele (que é geralmente cara). Mudar de um valor significa que ele não está mais no lugar de ser. Portanto, ao tentar acessá-lo no lugar de ser é algo a ser evitado. Uma discussão sobre quando e *como* mover que um valor está fora do escopo deste tópico. Neste tópico, precisamos apenas saber que um valor que é móvel é conhecido como um *rvalue* (ou *rvalue clássico*).

O "r" em "rvalue" é a abreviação de "direita" (como em, a direita-à lado de uma atribuição). Mas você pode usar rvalues e referências a rvalues, fora do atribuições. O "r" em "rvalues", em seguida, não é a coisa concentrar. Você só precisa compreender que o que chamamos de um rvalue é um valor que é móvel.

Lvalue, por outro lado, não é móvel, conforme mostrado nesta ilustração. Lvalue que movido seria enfrente a definição de *lvalue*e seria um problema inesperado para código que muito razoavelmente deve ser capaz de continuar a acessar o lvalue.

![Um rvalue é móvel; lvalue não é](images/is-movable.png)

Você não pode mover lvalue. Lá, mas *é* um tipo de glvalue (o conjunto de itens com identidade) que você pode mover&mdash;se você souber que você estiver fazendo (incluindo tomando cuidado para não acessá-lo após a movimentação)&mdash;que é o xvalue. Nós vai revisitar essa ideia mais uma vez abaixo, quando analisaremos o quadro completo das categorias de valor.
...
Nesse estágio, sabemos que tem identidade. E sabemos que é móvel e o que não é. Mas ainda não tenha sido chamado o conjunto de valores que *não* têm identidade. Esse conjunto é conhecido como o *prvalue*ou *rvalue pura*.
...
Um xvalue (um tipo de glvalue, mas também um tipo de rvalue) tem identidade e também é móvel. Isso pode ser lvalue mesma que você decidiu ao mover, pois a cópia é caro e você precisará ter cuidado para não acessá-lo posteriormente. Veja como você pode transformar lvalue em um xvalue.
...
Recolher múltiplo diferentemente referências em uma expressão para uma referência de lvalue.
...
- O motivo pelo qual as referências de encaminhamento (como `_Ty&&`) existe é *não* para otimização, mas para tirar o que você passe a eles e encaminhá-la no modo transparente e eficiente. Você provavelmente encontrar uma referência de encaminhamento somente se você escreve (ou investigar detalhadamente) código da biblioteca de&mdash;por exemplo, uma função de fábrica que encaminha argumentos de construtor.
