...
Cada expressão no C++ produz um valor que pertence a uma das categorias discutidas neste tópico. Há aspectos da linguagem C++, facilies e regras, que exigem uma compreensão adequada dessas categorias de valor e referências a elas. Por exemplo, pegar o endereço de um valor, copiar um valor, movendo um valor e um valor para outra função de encaminhamento. Este tópico não passar em todos os aspectos em detalhes, mas ela fornece informações fundamentais para ter uma compreensão sólida deles.

As informações neste tópico é quadro em termos de análise do Stroustrup das categorias de valor, as duas propriedades independentes de identidade e movability [Stroustrup, 2013].

## <a name="an-lvalue-has-identity"></a>Lvalue tem identidade
O que significa um valor ter *identidade*? Se você tiver (ou você pode executar) o endereço de memória de um valor e usá-lo com segurança, em seguida, o valor tem identidade. Dessa forma, você pode fazer mais do que comparar o conteúdo dos valores: você pode comparar ou diferenciá-las por identidade.

Um *lvalue* tem identidade. Agora é uma questão de interesse apenas histórico que o "l" no "lvalue" é a abreviação de "esquerda" (como à esquerda-à-lado de uma atribuição). Em C++, lvalue pode aparecer à esquerda *ou* à direita de uma atribuição. O "l" no "lvalues", em seguida, não realmente ajuda você a compreender nem definem o que eles estão. Você só precisa compreender o que o que chamamos de lvalue é um valor que tem identidade.
...
O "r" em "rvalue" é a abreviação de "direita" (como o direita-à-lado de uma atribuição). Mas você pode usar rvalues e referências a rvalues, fora do atribuições. O "r" em "rvalues", em seguida, não é a coisa concentrar. Você só precisa compreender o que o que chamamos de um rvalue é um valor que é móvel.

Lvalue, por outro lado, não móvel, conforme mostrado nesta ilustração. Lvalue que movido seria enfrente a definição de *lvalue*e seria um problema para o código que muito razoavelmente deve ser capaz de continuar a acessar o lvalue inesperado.

![Um rvalue é móvel; lvalue não é](images/is-movable.png)

Você não pode mover lvalue. Mas há *é* um tipo de glvalue (o conjunto de itens com identidade) que você pode mover&mdash;se você souber que você está fazendo (incluindo tenha cuidado para não acessá-lo após a movimentação)&mdash;que é o xvalue. Nós vai revisitar essa ideia mais uma vez abaixo, quando vemos a imagem completa de categorias de valor.
...
Um xvalue (um tipo de glvalue, mas também um tipo de rvalue) tem identidade e também é móvel. Isso pode ser lvalue mesma que você decidiu ao mover, pois a cópia é cara, e você vai tome cuidado para não acessá-lo posteriormente. Veja como você pode transformar lvalue em uma xvalue.
...
### <a name="im"></a>\!i\ & \!m
Tipo de valor que não tem identidade e não móveis é a combinação de um que ainda não abordadas. Mas, pode ignorar, porque a categoria não é uma ideia útil na linguagem C++.
...
Recolher vários ao contrário de referências em uma expressão para uma referência de lvalue.
