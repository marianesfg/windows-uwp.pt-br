...
Um *lvalue* tem identidade. Agora é uma questão de interesse apenas histórico que o "l" "lvalue" é a abreviação de "esquerda" (como à esquerda-à-lado de uma atribuição). Em C++, lvalue pode aparecer à esquerda *ou* à direita de uma atribuição. O "l" "lvalues", em seguida, não realmente ajuda você a compreender nem definir quais eles são. Você só precisa compreender que o que chamamos de lvalue é um valor que tem identidade.
...
O "r" em "rvalue" é a abreviação de "direita" (como o direita-à-lado de uma atribuição). Mas você pode usar rvalues e referências a rvalues fora atribuições. O "r" em "rvalues", em seguida, não é a coisa concentrar. Você só precisa compreender que o que chamamos de um rvalue é um valor que é móvel.
...
Você não pode mover lvalue. Mas há *é* um tipo de glvalue (o conjunto de coisas com identidade) que você pode mover&mdash;se você souber que você estiver fazendo (incluindo tenha cuidado para não acessá-lo após a movimentação)&mdash;que é o xvalue. Nós vai revisitar essa ideia mais uma vez abaixo, quando vemos a imagem completa de categorias de valor.

## <a name="rvalue-references-and-reference-binding-rules"></a>Referências de Rvalue e as regras de associação de referência
Esta seção apresenta a sintaxe para uma referência a um rvalue. Teremos de esperar para outro tópico ir para um tratamento considerável de encaminhamento e movendo de, mas esses são os problemas que são resolvidos por referências rvalue. Antes de examinarmos rvalue referências, porém, primeiro precisamos ser mais claras sobre `T&` &mdash;a coisa que estamos tiver anteriormente foi chamar apenas "uma referência". Ele é realmente "uma lvalue (não-const) referência", que se refere a um valor para o qual o usuário da referência pode gravar.
...
Em seguida, há referências const lvalue (`T const&`), que se referem a objetos para o qual o usuário a referência *não é possível* escrever (por exemplo, uma constante).
...
Neste estágio, sabemos que tem identidade. E sabemos que é móvel e o que não é. Mas ainda não tenha sido chamado o conjunto de valores que *não* têm identidades. Esse conjunto é conhecido como o *prvalue*ou *rvalue pura*.
...
Um xvalue (um tipo de glvalue, mas também um tipo de rvalue) tem identidade e também é móvel. Isso pode ser lvalue mesma que você decidiu mover porque a cópia é cara, e você vai tome cuidado para não acessá-lo posteriormente. Veja como você pode transformar lvalue em um xvalue.
...
### <a name="im"></a>\!i\ & \!m
O tipo de valor que não tem identidade e não móveis é a combinação de um que ainda não abordadas. Mas, pode ignorar, porque a categoria não é uma ideia útil na linguagem C++.

## <a name="reference-collapsing-rules"></a>Regras de recolhimento de referência
Várias referências como em uma expressão (uma referência lvalue para uma referência lvalue ou uma referência de rvalue para uma referência rvalue) cancelam a um outro out.

- `A& &` Recolhe em `A&`.
- `A&& &&` Recolhe em `A&&`.

Recolher várias diferentemente referências em uma expressão para uma referência de lvalue.
...
- O motivo pelo qual referências de encaminhamento (como `_Ty&&`) existe é *não* para otimização, mas para tirar o que você passa a eles e encaminhá-la no modo transparente e eficiente. Você provavelmente encontrar uma referência de encaminhamento somente se você escreve (ou investigar detalhadamente) código da biblioteca de&mdash;por exemplo, uma função de fábrica que encaminha argumentos de construtor.
