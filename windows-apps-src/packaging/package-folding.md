...
Em geral, ao separar alguns dos arquivos do seu aplicativo em outros pacotes (que não são pacotes de arquitetura), você não conseguirá acessar esses arquivos diretamente em relação ao local onde seu código é executado. Isso ocorre porque esses pacotes são instalados em diretórios diferentes de onde o pacote de arquitetura está instalado. Por exemplo, se você estiver fazendo um jogo e seu jogo está localizado em francês e alemão e é compilado para x86 e x64 máquinas, você deve ter esses arquivos de pacote de aplicativo dentro do pacote de aplicativo do seu jogo:
...
Quando o jogo é instalado no computador de um usuário, cada arquivo de pacote do aplicativo terá sua própria pasta **WindowsApps** directory. Portanto, para um usuário em francês executando o Windows de 64 bits, seu jogo terá a seguinte aparência:
