...
description: "Este white paper descreve técnicas de migração de aplicativos .NET e Win32 de recursos herdados do Win32 para recursos MRT quando empacotados como pacotes AppX."
ms.author: ptorr
ms.date: 03/09/2017
ms.topic: article
ms.prod: windows
ms.technology: uwp
keywords: "windows 10, uwp, mrt, pri. recursos, jogos, centennial, conversor de aplicativos de área de trabalho, mui, assembly satélite"
...
---
# <a name="migrating-legacy-resources-to-the-windows-10-resource-management-platform"></a>Migrando recursos herdados para a plataforma de gerenciamento de recursos do Windows 10

## <a name="executive-summary"></a>Síntese

Os aplicativos Win32 geralmente são localizados para idiomas diferentes, expandindo totalmente o mercado ao qual se destinam<a name="footnote1_ref" href="#footnote1"><sup>1</sup></a>. Há muitas maneiras de localizar aplicativos Win32 tradicionais, mas o Windows 8 introduziu uma [novo sistema de gerenciamento de recursos](https://msdn.microsoft.com/en-us/library/windows/apps/jj552947.aspx) (referenciado como "MRT" neste documento <a name="footnote2_ref" href="#footnote2"><sup>2</sup></a>) que funciona entre linguagens de programação, entre tipos de aplicativos e fornece uma funcionalidade que transcende a simples localização. 

Combinado com a implantação baseada em AppX (ou seja, na Windows Store), o MRT pode fornecer automaticamente os recursos mais aplicáveis de um determinado usuário/dispositivo que minimiza o tamanho do download e da instalação do aplicativo. Essa redução de tamanho pode ser significativa para aplicativos com uma grande quantidade de conteúdo localizado, talvez na ordem de vários *gigabytes* para jogos AAA. Os benefícios adicionais do MRT incluem listagens localizadas no Shell do Windows e na Windows Store, lógica de fallback automática quando o idioma preferencial de um usuário não corresponde aos recursos disponíveis.
...
Em muitas situações, você pode continuar usando o código-fonte e os formatos de localização existentes, integrando-se, ao mesmo tempo, ao MRT para resolver recursos em tempo de execução e minimizar os tamanhos dos downloads; não é uma abordagem "tudo ou nada". A tabela a seguir resume o trabalho e o custo estimado/benefício de cada estágio <a name="footnote3_ref" href="#footnote3"><sup>3</sup></a>:
...
<td>Descobrir o mínimo de trabalho necessário para que o conteúdo localizado apareça no Shell do Windows e na Windows Store</td>
<td>Pequeno</td>
</tr>
<tr>
...
Após concluir as fases 0 a 3 acima, você terá um "pacote" de aplicativos que poderá ser enviado à Windows Store e minimizará o tamanho do download e da instalação para os usuários, omitindo os recursos não necessários (por exemplo, os idiomas que eles não falam). É possível fazer melhorias adicionais no tamanho e na funcionalidade do aplicativo através da execução de uma etapa final. 
...
0. No arquivo XML, adicione o conteúdo a seguir, no qual o <span style="background-color: yellow">texto realçado</span> é substituído pelo texto apropriado para o aplicativo, no idioma padrão<a name="footnote5_ref" href="#footnote5"><sup>5</sup></a>
...
Por outro lado, se o aplicativo usar uma estrutura de arquivo simples com um único diretório `Resources` contendo os arquivos `english_strings.dll` e `french_strings.dll`, ele **não** fará um mapeamento satisfatório para o MRT. Uma estrutura melhor seria um diretório `Resources` com subdiretórios e arquivos `en\strings.dll` e `fr\strings.dll`
<a name="footnote8_ref" href="#footnote8"><sup>8</sup></a>. Observe que ainda é possível usar o MRT e os benefícios do empacotamento AppX, mesmo se você não puder seguir essa convenção de nomenclatura de arquivo; isso apenas requer mais trabalho. Um white paper futuro abordará esse caso. 
...
O tempo de execução do .NET acionará o evento `AssemblyResolve` sempre que não conseguir encontrar as DLLs de recurso; nesse momento, o manipulador de eventos fornecido localizará o arquivo desejado por meio do MRT e retornará o assembly<a name="footnote9_ref" href="#footnote9"><sup>9</sup></a>.
...
Clique no número para retornar ao local de origem no documento.

<a name="footnote1" href="#footnote1_ref">1.</a> Este documento não fornece nenhuma motivação adicional para a localização propriamente; presume-se que o leitor já entende isso.

<a name="footnote2" href="#footnote2_ref">2.</a> Antigamente, a sigla significava "Modern Resource Technology", mas o termo "Modern" deixou de ser utilizado. O gerenciador de recursos também pode ser conhecido como MRM (Modern Resource Manager) ou PRI ( Índice de Recurso do Pacote).

<a name="footnote3" href="#footnote3_ref">3.</a> Esta tabela não inclui tarefas de não localização, como fornecimento de ícones de aplicativos de alta resolução ou de alto contraste. Consulte o MSDN para obter mais informações sobre como fornecer vários ativos de blocos, ícones etc.

<a name="footnote4" href="#footnote4_ref">4.</a> As especificações do fator de escala não são abordadas aqui

<a name="footnote5" href="#footnote5_ref">5.</a> Observe que há restrições quanto aos tamanhos de algumas dessas cadeias de caracteres; consulte [o tópico **VisualElements** no MSDN](https://msdn.microsoft.com/en-us/library/windows/apps/br211471.aspx) para obter mais informações 

<a name="footnote6" href="#footnote6_ref">6.</a> Não há nenhuma exigência para usar exatamente estes nomes de recurso; você pode escolher o nome, desde que ele corresponda exatamente ao que estiver no arquivo `.resw`.

<a name="footnote7" href="#footnote7_ref">7.</a> Você pode escolher qualquer outro diretório, mas não se esqueça de substituí-lo nos comandos futuros

<a name="footnote8" href="#footnote8_ref">8.</a> Também é possível usar o mesmo nome de arquivo base, mas com qualificadores inseridos, como `strings.lang-en.dll` e `strings.lang-fr.dll`, mas, como usar diretórios com os códigos de idioma é conceitualmente mais simples, vamos nos concentrar nisso.

<a name="footnote9" href="#footnote9_ref">9.</a> Observe que, se o aplicativo já tiver um manipulador `AssemblyResolve` para outros fins, será necessário integrar o código de resolução de recurso ao código existente.
