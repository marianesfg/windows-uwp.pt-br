...
**Observação**  O DRM baseado em hardware tem suporte somente em determinado hardware com uma versão do Windows 10 do firmware desse hardware. Consulte as [Regras de Conformidade e Robustez do PlayReady](http://www.microsoft.com/playready/licensing/compliance/) para obter mais informações sobre as garantias fornecidas.
...
Este tópico fornece uma breve lista de itens que devem ser considerados ao desenvolver aplicativos projetados para usar DRM de hardware.

-   Não há suporte para PMP (Processo de Mídia Protegida)
-   Suporte ao OPL (Nível de Proteção de Saída) 270 (não é permitida resolução inferior). A Microsoft recomenda que conteúdo de alta definição para DRM de hardware tenha um OPL superior a 270 (embora não seja necessário). Um OPL superior a 270 significa que a HDCP é necessária. Além disso, a Microsoft recomenda que você defina a HDCP tipo 1 (versão 2.2 ou posterior).
-   Ao contrário do DRM de software, proteções de saída são impostas em todos os monitores com base no monitor de menor capacidade. Por exemplo, se o usuário tiver dois monitores conectados, em que um dos monitores oferece suporte à uma HDCP e o outro não, haverá falha na reprodução se a licença exigir uma HDCP, mesmo se o conteúdo só estiver sendo renderizado no monitor que oferece suporte à HDCP. No DRM de software, o conteúdo é reproduzido contanto que esteja sendo renderizado somente no monitor que oferece suporte à HDCP.
-   Não há garantia de que o DRM de hardware seja usado pelo cliente nem que seja seguro, a menos que as seguintes condições sejam atendidas pelas licenças e chaves do conteúdo:
    -   O áudio não deve estar criptografado ou estar criptografado com uma chave de conteúdo diferente do vídeo. A Microsoft recomenda que o áudio não esteja criptografado para melhorar o desempenho de reprodução.
    -   A licença usada para a chave de conteúdo de vídeo deve ter um nível de segurança de 3000.
