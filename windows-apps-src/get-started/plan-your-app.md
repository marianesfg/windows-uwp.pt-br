---
title: Criar um aplicativo complexo da UWP (Plataforma Universal do Windows)
...
**Concentre-se em seu aplicativo**
...
-   Crie um storyboard do fluxo: como os usuários devem navegar pela interface do usuário para concluir o fluxo?
-   Crie um protótipo: experimente o fluxo com um protótipo rápido.
...
Para novos desenvolvedores, nunca é demais destacar que: Criar ótimos aplicativos é um processo iterativo. Recomendamos que você prototipe cedo e frequentemente. Como qualquer esforço criativo, os melhores aplicativos são o produto de um processo intensivo de teste e erro.
...
-   Crie um protótipo: exercite os recursos para garantir que eles façam o que você precisa.

**Contratos do aplicativo**  Seu aplicativo pode participar de contratos do aplicativo que permitem amplos fluxos dos usuários entre aplicativos e entre recursos.
...
**Interessante e novo**  Seu aplicativo deve ser percebido como novo e interessante para os usuários com estas experiências padrão:
...
-   **Blocos de aplicativo**  Fornece atualizações recentes e relevantes para atrair os usuários de volta para o seu aplicativo. Há mais sobre isso na próxima seção. Saiba mais sobre [blocos de aplicativos](../design/shell/tiles-and-notifications/creating-tiles.md).
...
-   **Roaming**  Crie uma experiência contínua por meio de dispositivos fazendo roaming de dados que permite que os usuários retomem uma tarefa exatamente no local em que pararam e preserva a experiência do usuário com que eles mais se importam, independentemente do dispositivo que estejam usando. Facilite o uso do seu aplicativo em qualquer lugar—na cozinha, no computador da família ou de trabalho, no tablet pessoal, e outros fatores forma—mantendo configurações e estados com roaming. Saiba mais sobre [gerenciamento de dados de aplicativos](../design/app-settings/store-and-retrieve-app-data.md) e consulte [Diretrizes de dados de aplicativo em roaming](https://docs.microsoft.com/windows/uwp/design/app-settings/store-and-retrieve-app-data).
-   **Blocos de usuário**   Deixe seu aplicativo mais pessoal para os usuários carregando a imagem de bloco de usuário deles ou permita que eles definam conteúdo do seu aplicativo como bloco pessoal no Windows.

**Capacidades de dispositivo**  Seu aplicativo deve beneficiar-se ao máximo das capacidades dos dispositivos de hoje.
...
-   Pesquisa: os usuários pesquisam e encontram atividades ou destinos de itinerários compartilhados ou públicos de outras pessoas, que eles podem incluir em suas próprias viagens.
...
-   Zoom semântico: os usuários navegam pela linha do tempo de seu itinerário e aplicam zoom para ver mais detalhes da longa lista de atividades que planejaram.
-   Blocos do usuário: os usuários escolhem a imagem que querem exibir ao compartilharem suas viagens com amigos.
...
**De quais comandos e superfícies de interface do usuário você precisa?**   Examine os fluxos que você identificou antes. Para cada fluxo, crie um esboço das etapas que os usuários devem seguir.
...
**Como você deve organizar comandos?**   Use a descrição do fluxo para identificar comandos em potencial que você precisará projetar. Depois, pense nos locais onde usar estes comandos em seu aplicativo.
...
-   **Se não puder, use o conteúdo.** Coloque comandos em uma destas superfícies de IU se você não puder usar o conteúdo:
    ...
    -   Na [barra de comandos](https://docs.microsoft.com/windows/uwp/controls-and-patterns/app-bars): você deve colocar a maioria dos comandos na barra de comandos, que costuma estar oculta até que o usuário toque para deixá-la invisível.
    -   Na tela do aplicativo: se o usuário estiver em uma página ou modo de exibição que tenha uma única finalidade, você poderá oferecer comandos para essa finalidade diretamente na tela. Deve haver muito pouco desses comandos presentes.
    -   Em um [menu de contexto](https://docs.microsoft.com/windows/uwp/controls-and-patterns/menus): você pode usar menus de contexto para ações da área de transferência (como recortar, copiar e colar) ou para comandos que se aplicam ao conteúdo que não pode ser selecionado (como a adicionar uma tachinha a um local no mapa).

**Decida como dispor seu aplicativo em cada exibição.**   O Windows é compatível com as orientações de paisagem e retrato e ao redimensionamento de aplicativos para qualquer largura, de tela inteira até uma largura mínima. Você quer que o seu aplicativo tenha uma boa aparência e funcione perfeitamente em qualquer site, em qualquer tela, em ambas as orientações. Isso significa que você precisa planejar o layout dos elementos da interface para diferentes tamanhos e exibições. Ao fazer isso, a IU do seu aplicativo muda de maneira fluida para atender às necessidades e preferências do usuário.

![designs móveis e de computador para um aplicativo](images/ux-budgettracker1-md-notablet.png)

Para obter mais informações sobre a criação de diferentes tamanhos de tela, confira [Tamanhos de tela e pontos de interrupção para um design responsivo](https://docs.microsoft.com/windows/uwp/design/layout/screen-sizes-and-breakpoints-for-responsive-design).
