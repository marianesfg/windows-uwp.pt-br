...
Microsoft Visual Studio permite implantar e depurar seus aplicativos Plataforma Universal do Windows (UWP) em uma variedade de dispositivos Windows 10. O Visual Studio manipulará o processo de criação e registro do aplicativo no dispositivo de destino.
...
- Um destino de **Emulador** será inicializado e implantará o aplicativo em um emulador com a configuração especificada no nome. Os emuladores só estão disponíveis em máquinas habilitadas para Hyper-V que executam Windows 8.1 ou além.
...
Para retornar a esta caixa de diálogo, você pode abrir as propriedades do projeto e ir para a guia **depurar** . A partir daí, selecione **Localizar** ao lado de **computador remoto:**
...
Para especificar um destino de computador remoto para C++ um aplicativo JavaScript UWP:
...
No lançamento do Visual Studio 2015 atualização 3 e da atualização de aniversário do Windows 10, há novas opções avançadas de implantação remota para determinados dispositivos Windows 10. As opções de implementação remota avançada podem ser encontradas no menu **Depuração** das propriedades do projeto.
...
- Exemplo 1 (pasta local do layout, acessível como um compartilhamento de rede):
  ...
  - **Caminho de registro de pacote** = `\\NETWORK-SHARE\Layouts\App1`
...
No dispositivo remoto, o layout é registrado no seguinte local padrão, dependendo da família do dispositivo: `Xbox: \\MY-DEVKIT\DevelopmentFiles\XrfsFiles`-este é um symlink para o caminho de **registro do pacote** o PC não usa um symlink e registra diretamente o **caminho de registro do pacote**

## <a name="debugging-options"></a>Opções de depuração

No Windows 10, o desempenho de inicialização dos aplicativos UWP é melhorado por meio da inicialização proativa e, em seguida, da suspensão de aplicativos em uma técnica chamada [pré-inicialização](https://docs.microsoft.com/windows/uwp/launch-resume/handle-app-prelaunch). Muitos aplicativos não precisam fazer nada especial para trabalhar nesse modo, mas alguns aplicativos talvez precisem ajustar seu comportamento. Para ajudar a depurar problemas nesses caminhos de código, você pode iniciar o aplicativo do Visual Studio no modo de pré-inicialização.

A depuração tem suporte tanto de um projeto do Visual Studio (**Debug** -&gt; **outros destinos de depuração** -&gt; **depuração de pré-inicialização de aplicativo universal do Windows**) e para aplicativos já instalados no computador (**depuração** -&gt; **outros destinos de depuração** -&gt; **depurar pacote do aplicativo instalado** marcando a caixa de seleção **Ativar aplicativo com pré-inicialização** ). Para obter mais informações, consulte [Depurar pré-inicialização de UWP](https://blogs.msdn.com/b/visualstudioalm/archive/2015/11/30/debug-uwp-prelaunch-with-vs2015.aspx).
...
- **Permitir loopback de rede local**
  ...
  Por motivos de segurança, um aplicativo UWP que foi instalado de modo padrão não pode fazer chamadas de rede para o dispositivo em que estiver instalado. Por padrão, a implantação do Visual Studio cria uma isenção da regra para o aplicativo implantado. Esse isolamento permite que você teste procedimentos de comunicação em uma única máquina. Antes de enviar seu aplicativo para o Microsoft Store, você deve testar seu aplicativo sem a isenção.

  Para remover a isenção de loopback de rede do aplicativo:

  - Na página C# de propriedades de **depuração** e Visual Basic, desmarque a caixa de seleção **permitir loopback de rede local** .
  - Na página da propriedade **Depuração** do JavaScript e do C++, defina o valor **Permitir Loopback de Rede Local** como **Não**.

- **Não iniciar, mas depurar meu código quando iniciar/iniciar o aplicativo**
