...
Microsoft Visual Studio permite que você implantar e depurar seus aplicativos de plataforma Universal do Windows (UWP) em uma variedade de dispositivos Windows 10. O Visual Studio manipulará o processo de criação e registro do aplicativo no dispositivo de destino.
...
-   **Simulador** implantará o aplicativo em um ambiente simulado em seu computador de desenvolvimento atual. Essa opção só estará disponível se seu aplicativo **mínima da plataforma de destino. Versão** é menor que ou igual ao sistema operacional no computador de desenvolvimento.
-   **Máquina Local** implantará o aplicativo no computador de desenvolvimento atual. Essa opção só estará disponível se seu aplicativo **mínima da plataforma de destino. Versão** é menor que ou igual ao sistema operacional no computador de desenvolvimento.
...
-   Um destino de **Emulador** será inicializado e implantará o aplicativo em um emulador com a configuração especificada no nome. Emuladores são somente disponível no Hyper-V habilitada máquinas que executam o Windows 8.1 ou posterior.
...
Para especificar um destino do computador remoto para um aplicativo C++ ou JavaScript UWP:
...
Para implantar em um computador remoto pré-atualização dos criadores, o computador de destino deve ter as Ferramentas Remotas do Visual Studio instaladas. PC remoto também deve estar executando uma versão do Windows que é maior que ou igual aos seus aplicativos **mínima da plataforma de destino. Versão** propriedade. Após instalar as ferramentas remotas, você deve iniciar o depurador remoto no computador de destino.
...
Quanto da versão do Visual Studio 2015 atualização 3 e a atualização de aniversário do Windows 10, há nova implantação remota avançada para determinadas opções de dispositivos Windows 10. As opções de implementação remota avançada podem ser encontradas no menu **Depuração** das propriedades do projeto.
...
* Ter o Visual Studio 2015 atualização 3 ou alguns mais recente versão do Visual Studio instalado com as ferramentas do Windows 10 1.4.1 ou posterior (que inclui o SDK de atualização de aniversário do Windows 10) é recomendável que você use a versão mais recente do Visual Studio com as atualizações para garantir que você obtenha todos os o recursos de desenvolvimento e de segurança mais recentes.
...
No dispositivo remoto, o layout é registrado no seguinte local padrão, dependendo da família do dispositivo: `Xbox: \\MY-DEVKIT\DevelopmentFiles\XrfsFiles` -este é um symlink para o **caminho de registro do pacote** PC não usa um symlink e, em vez disso, diretamente registra o **caminho de registro do pacote**
...
WinDbg é um depurador avançado fornecido como parte do conjunto de Ferramentas de Depuração para Windows, que está incluído no [SDK do Windows](https://go.microsoft.com/fwlink/p/?LinkID=271979). A instalação do SDK do Windows permite que você instale as Ferramentas de Depuração para Windows como um produto autônomo. Embora seja altamente útil para a depuração de código nativo, não recomendamos o WinDbg para aplicativos escritos em código gerenciado ou HTML5.
