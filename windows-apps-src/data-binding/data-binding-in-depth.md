...
| RelativeFonte: TemplatedParent | Não necessário | `{Binding <path>, RelativeSource={RelativeSource TemplatedParent}}` | {X: Bind} TargetType em ControlTemplate indica associação pai de modelo. Para {Binding} associação de modelo Regular pode ser usada nos modelos de controle para a maioria dos usos. Mas use TemplatedParent onde você precisa usar um conversor ou uma associação bidirecional.< | 
