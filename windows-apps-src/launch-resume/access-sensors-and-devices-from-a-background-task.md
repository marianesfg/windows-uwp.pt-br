...
Tarefas em segundo plano que usam o [**DeviceUseTrigger**](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Background.DeviceUseTrigger) permitem que o seu aplicativo se comunique através de muitos protocolos/APIs, sendo que a maioria deles não tem suporte para tarefas em segundo plano disparadas pelo sistema. Há suporte em aplicativos Universal do Windows para os seguintes itens.

| Protocol         | DeviceUseTrigger em um aplicativo Universal do Windows                                                                                                                                                    |
|------------------|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------:|
...
| API de sensores      | ![deviceservicingtrigger oferece suporte a apis de sensores universais](images/ap-tools.png) (limitado a sensores na [família de dispositivos universal](https://docs.microsoft.com/windows/uwp/get-started/universal-application-platform-guide)) |
...
O seu aplicativo executará operações de sincronização e atualização em código que é executado como parte de uma tarefa em segundo plano. Esse código está incorporado em uma classe do Windows Runtime que implementa [**IBackgroundTask**](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Background.IBackgroundTask) (ou em uma página dedica JavaScript ou a aplicativos JavaScript). Para usar uma tarefa em segundo plano do [**DeviceUseTrigger**](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Background.DeviceUseTrigger), o seu aplicativo deve declarar isso no arquivo de manifesto do aplicativo, como faz para tarefas em segundo plano disparadas pelo sistema.

Neste exemplo de manifesto de pacote do aplicativo, **DeviceLibrary.SyncContent** é o ponto de entrada para uma tarefa em segundo plano que usa o [**DeviceUseTrigger**](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Background.DeviceUseTrigger).
...
Para usar o [**DeviceUseTrigger**](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Background.DeviceUseTrigger), siga estas etapas básicas. Para saber mais sobre tarefas em segundo plano, consulte [Support your app with background tasks](support-your-app-with-background-tasks.md).
...
2.  Quando o seu aplicativo iniciar, ele criará e configurará um objeto trigger do tipo [**DeviceUseTrigger**](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Background.DeviceUseTrigger), e armazenará a instância do gatilho para uso futuro.
...
4.  Quando o seu aplicativo precisar disparar a tarefa em segundo plano, ele deve chamar primeiro o [**RequestAccessAsync**](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.backgroundexecutionmanager.requestaccessasync) para verificar se o aplicativo é capaz de solicitar uma tarefa em segundo plano.
...
**Importante**    considerar os seguintes pontos importantes ao usar o [ **DeviceUseTrigger**](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Background.DeviceUseTrigger):

-   A capacidade de disparar programaticamente o plano de fundo de tarefas que usam o [ **DeviceUseTrigger** ](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Background.DeviceUseTrigger) foi introduzida no Windows 8.1 e Windows Phone 8.1.
...
| O número máximo de tarefas em segundo plano do [DeviceUseTrigger](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Background.DeviceUseTrigger) ainda não foi alcançado no dispositivo (em que o seu app está sendo executado). | **Família de dispositivos da área de trabalho**: Um número ilimitado de tarefas pode ser registrado e executado em paralelo. **Família de dispositivos móveis**: 1 tarefa em um dispositivo de 512 MB; Caso contrário, 2 tarefas podem ser registradas e executadas em paralelo. |
...
4.  Escolha objeto aberto de dispositivo ou sensor quando disparar sua tarefa em segundo plano do [**DeviceUseTrigger**](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Background.DeviceUseTrigger) para que tais dispositivos e sensores possa ser abertos e usados pela sua tarefa em segundo plano.
...
Para cancelar uma tarefa em execução no segundo plano a partir do seu aplicativo em primeiro plano, use o método Unregister no objeto [**BackgroundTaskRegistration**](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Background.BackgroundTaskRegistration) que você usa no seu aplicativo para registrar a tarefa em segundo plano do [**DeviceUseTrigger**](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Background.DeviceUseTrigger). Cancelar o registro da sua tarefa em segundo plano usando o método [**Unregister**](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.backgroundtaskregistration.unregister) em **BackgroundTaskRegistration** levará a infraestrutura da tarefa em segundo plano a cancelá-la.

O método [**Unregister**](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.backgroundtaskregistration.unregister), adicionalmente, leva um valor Booleano verdadeiro ou falso para indicar se instâncias em execução na sua tarefa em segundo plano devem ser canceladas sem deixar que elas sejam concluídas. Para obter mais informações, consulte a referência de API para **Unregister**.

Além de [**Unregister**](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.backgroundtaskregistration.unregister), o seu aplicativo também precisará chamar [**BackgroundTaskDeferral.Complete**](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.backgroundtaskdeferral.complete). Isso informa ao sistema que a operação assíncrona associada a uma tarefa em segundo plano foi concluída.
