...
Se você estiver pensando em usar um gatilho de aplicativo para estender a duração de seu processo em primeiro plano, considere usar [Execução estendida](run-minimized-with-extended-execution.md) . O gatilho de aplicativo é projetado para a criação de um processo separadamente hospedado para funcionar em. O trecho de código a seguir registra um gatilho em segundo plano fora do processo.
