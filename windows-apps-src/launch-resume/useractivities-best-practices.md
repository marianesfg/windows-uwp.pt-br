...
* **Aplicativos baseados em documento** — criar uma atividade por documento, com um ou mais itens de histórico refletindo períodos de uso. É importante para a atividade de atualização, como as alterações são feitas no documento.
* **Jogos** — criar uma atividade para cada Salvar jogo ou o mundo. Se o jogo dá suporte a apenas uma única sequência de níveis, você pode publicar novamente a mesma atividade ao longo do tempo, embora talvez você queira atualizar os dados de conteúdo para mostrar o progresso ou realizações mais recentes.
* **Aplicativos de utilidade** — se não houver nada em seu aplicativo que os usuários precisariam deixar e retomar, você não precisa usar atividades do usuário. Um bom exemplo é um aplicativo simples como calculadora.
...
* Use as informações de texto complementares com moderação. 
  * Evite preenchendo o cartão com texto e usar apenas as informações complementares que ajuda os usuários a encontrar a atividade direita ou reflete as informações de estado (por exemplo, o progresso atual em uma determinada tarefa).
