...
Windows 10, versão 1803 (10.0; Build 17134) em diante, seu aplicativo UWP pode optar por dar suporte a várias instâncias. Se uma instância de um aplicativo UWP de várias instâncias estiver em execução, e uma solicitação de ativação subsequente chegar, a plataforma não ativará a instância existente. Em vez disso, ela criará uma nova instância, executada em um processo separado.

> [!IMPORTANT]
> Várias instâncias tem suporte para aplicativos JavaScript, mas não é de redirecionamento de várias instâncias. Uma vez que não há suporte para várias instâncias de redirecionamento para aplicativos JavaScript, o [ **AppInstance** ](/uwp/api/windows.applicationmodel.appinstance) classe não é útil para tais aplicativos.
...
Dois modelos são instalados: **Aplicativo UWP de várias instâncias**, que fornece o modelo para criar um aplicativo de várias instâncias, e **aplicativo UWP de redirecionamento de várias instâncias**, que fornece lógica adicional que você pode se basear para ou iniciar uma nova instância ou ative seletivamente uma instância que já foi iniciada. Por exemplo, talvez você queira que uma instância por vez editando o mesmo documento, para que você colocar a instância que tem esse arquivo abrir para o primeiro plano em vez de iniciar uma nova instância.

Adicionam os dois modelos `SupportsMultipleInstances` para o `package.appxmanifest` arquivo. Observe o prefixo do namespace `desktop4` e `iot2`: apenas os projetos destinados a desktop, ou projetos de Internet das coisas (IoT), dar suporte a várias instâncias.
...
Para ver isso em ação, assista a este vídeo sobre como criar várias instâncias de aplicativos UWP.
...
O [ **AppInstance.RecommendedInstance** ](/uwp/api/windows.applicationmodel.appinstance.recommendedinstance) propriedade representa a instância fornecida pelo shell preferencial para esta solicitação de ativação, se houver um (ou `null` se houver um). Se o shell fornece uma preferência, em seguida, você pode redirecionar a ativação a essa instância ou pode ignorá-lo se você escolher.
...
- Os aplicativos podem usar o [ **ResourceGroup** ](https://docs.microsoft.com/windows/uwp/launch-resume/declare-background-tasks-in-the-application-manifest) declaração em seu manifesto para agrupar várias tarefas em segundo plano para o mesmo host. Entra em conflito com várias instâncias, onde cada ativação entra em um host separado. Portanto, um aplicativo não pode declarar as duas `SupportsMultipleInstances`e `ResourceGroup` em seu manifesto.

## <a name="sample"></a>Exemplo

Ver [exemplo de várias instâncias](https://aka.ms/Kcrqst) para obter um exemplo de redirecionamento de ativação de várias instâncias.
