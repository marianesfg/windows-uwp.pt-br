...
description: Saiba como obter e interpretar os dados de código de barras que você verificam.
ms.author: elcowle
ms.date: 08/29/2018
ms.topic: article
ms.prod: windows
...
Essa propriedade deve incluir um caractere de Simbologia caso um seja retornado pelo scanner ( **por exemplo, de um para** um UPC). Ele também deve incluir dígitos de verificação se eles estiverem presentes no rótulo e retornados pelo scanner. (Observe que caracteres de Simbologia e dígitos de verificação podem ou não ser presentes, dependendo da configuração de scanner. O scanner retornará-los a se presente, mas não gerar ou calcule se eles estiverem ausentes.)
