...
description: Este artigo fornece uma visão geral do servidor de perfil de atributo genérico (GATT) Bluetooth para aplicativos da plataforma Universal do Windows (UWP), juntamente com o código de exemplo para casos de uso comuns.
ms.date: 02/08/2017
ms.topic: article
...
Este artigo demonstra Bluetooth GATT (atributo genérico) Server APIs para aplicativos da plataforma Universal do Windows (UWP), juntamente com o código de exemplo para tarefas comuns de servidor GATT: 
...
Se o atributo é definido pelo Bluetooth SIG definidos e padrão, ele também tem uma ID de curta de 16 bits correspondente (por exemplo, o UUID do nível de bateria é 0000**2A19**-0000-1000-8000-00805F9B34FB e a ID curta é 0x2A19). Esses UUIDs padrão podem ser vistos no [GattServiceUuids](https://msdn.microsoft.com/en-us/library/windows/apps/windows.devices.bluetooth.genericattributeprofile.gattserviceuuids.aspx) e [GattCharacteristicUuids](https://msdn.microsoft.com/en-us/library/windows/apps/windows.devices.bluetooth.genericattributeprofile.gattcharacteristicuuids.aspx).
...
> Observe que a difusão não é suportada no momento.  Especificando a transmissão GattCharacteristicProperty resultará em uma exceção.
...
```
> Serviços primários são de nível superior da árvore de GATT. Serviços primários contêm características, bem como outros serviços (chamados 'Incluído' ou serviços secundários). 

Agora, preencha o serviço com o descritores e características necessárias:
