---
Description: Você pode usar o método SendRequestAsync para enviar solicitações para o Microsoft Store para operações que ainda não têm uma API disponível no SDK do Windows.
...
O exemplo a seguir mostra o formato dos dados JSON para passar a *parametersAsJson*. O campo *type* deve ser atribuído à cadeia de caracteres *GetRemoteVariables*. Atribua o campo *ProjectId* à ID do projeto no qual você definiu as variáveis remotas no Partner Center.
