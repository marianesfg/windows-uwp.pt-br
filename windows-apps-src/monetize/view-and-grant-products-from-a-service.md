...
4.  No seu aplicativo do cliente Windows, [Crie uma chave de ID da Microsoft Store](#step-4) que representa a identidade do usuário atual e secreta esta tecla de volta ao seu serviço.
...
* **Seu serviço**. Este é um aplicativo executado de forma segura no contexto do seu ambiente de negócios e pode ser implementado usando qualquer plataforma de desenvolvimento que você escolher. O serviço é responsável para criar os tokens de acesso do Azure AD necessária para o cenário e para chamar os URIs do restante da coleção Microsoft Store API e a API de compra.
* **Seu aplicativo do Windows de cliente**. Esse é o aplicativo para o qual você deseja acessar e gerenciar informações de qualificação do cliente (incluindo complementos para o aplicativo). Este aplicativo é responsável por criar as chaves do repositório de ID da Microsoft que você precisará chamar o conjunto do Microsoft Store API e API de compra do seu serviço.

<span id="step-1"/>

## <a name="step-1-configure-an-application-in-azure-ad"></a>Etapa 1: Configurar um aplicativo no Azure AD.

Antes de poder usar a coleção Microsoft Store API ou API de compra, você deve criar um aplicativo Web do Windows Azure AD, recuperar o locatário ID e a ID de aplicativo para o aplicativo e gerar uma chave. O aplicativo Web do Windows Azure AD representa o serviço do qual você deseja chamar a coleção Microsoft Store API ou API de compra. É necessário o ID do inquilino, ID de aplicativo e a chave para gerar os tokens de acesso do Azure AD que você precisa para chamar a API.

> [!NOTE]
> Você precisa executar somente as tarefas nesta seção uma vez. Depois que você atualizar seu manifesto de aplicativo do Windows Azure AD e você tiver seu ID do inquilino, segredo de cliente e a ID do aplicativo, você pode reutilizar esses valores sempre que você precisa criar um novo token de acesso do Azure AD.

1.  Se você ainda não tiver feito isso, siga as instruções em [Aplicativos integrando com o Windows Azure Active Directory](https://docs.microsoft.com/azure/active-directory/develop/active-directory-integrating-applications) para registrar um **Web app / API** aplicativos com o Azure AD.
    > [!NOTE]
    > Ao registrar seu aplicativo, você deve escolher **Web app / API** conforme o tipo de aplicativo para que você possa recuperar uma chave (também chamada de um *segredo do cliente*) para o seu aplicativo. Para chamar a API de coleção ou a API de compra da Microsoft Store, você deverá fornecer o segredo do cliente quando solicitar um token de acesso do Azure AD em uma etapa posterior.

2.  No [Portal de gerenciamento do Windows Azure](https://portal.azure.com/), navegue até o **Azure Active Directory**. Selecione seu diretório, clique **os registros do aplicativo** no painel de navegação esquerdo e, em seguida, selecione seu aplicativo.
3.  É exibida a página de registro principal do aplicativo. Nessa página, copie o valor de **ID do aplicativo** para uso posterior.
4.  Crie uma chave que será necessário posteriormente (Isso é tudo chamado um *segredo do cliente*). No painel esquerdo, clique em **configurações** e, em seguida, **chaves**. Nessa página, conclua as etapas para [criar uma chave](https://docs.microsoft.com/azure/active-directory/develop/active-directory-integrating-applications#to-add-application-credentials-or-permissions-to-access-web-apis). Copie essa chave para uso posterior.
5.  Adicione vários URIs de audiência necessária ao seu [manifesto do aplicativo](https://docs.microsoft.com/azure/active-directory/develop/active-directory-application-manifest). No painel esquerdo, clique em **manifesto**. Clique em **Editar**, substitua o `"identifierUris"` seção com o seguinte texto e clique em **Salvar**.
...
## <a name="step-2-associate-your-azure-ad-application-id-with-your-client-app-in-windows-dev-center"></a>Etapa 2: Associar sua ID de aplicativo do Windows Azure AD ao seu aplicativo cliente no Windows Dev Center

Antes de poder usar a coleção Microsoft Store API ou API para configurar a propriedade e compras para seu aplicativo ou um complemento de compra, você deverá associar sua ID de aplicativo do Windows Azure AD do aplicativo (ou o aplicativo que contém o complemento) no painel do Centro de desenvolvimento.
...
2.  Vá para os **Serviços** &gt; **compras e conjuntos de produto** da página e insira sua ID de aplicativo do Windows Azure AD em um dos campos de **ID de cliente** disponíveis.
...
* Para os parâmetros *client\_id* e *client\_secret* , especifique a ID do aplicativo e o segredo do cliente para o seu aplicativo recuperado a partir do [Portal de gerenciamento do Windows Azure](http://manage.windowsazure.com). Esses dois parâmetros são necessários para criar um token de acesso com o nível de autenticação exigido pela API de compra ou API de coleção da Microsoft Store.
...
2.  No código do seu app, chame um desses métodos para recuperar uma chave de ID da Microsoft Store:
  ...
  * Se seu aplicativo usa a classe [CurrentApp](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Store.CurrentApp) no namespace [ApplicationModel](https://docs.microsoft.com/uwp/api/windows.applicationmodel.store) para gerenciar as compras no aplicativo, use o método [CurrentApp.GetCustomerCollectionsIdAsync](https://docs.microsoft.com/uwp/api/windows.applicationmodel.store.currentapp.getcustomercollectionsidasync).
    ...
    Passe o token de acesso do Azure AD para o parâmetro *serviceTicket* do método. Se você mantiver IDs de usuário anônimo no contexto de serviços que você gerenciar como o fornecedor do aplicativo atual, você também pode passar uma ID de usuário para o parâmetro *publisherUserId* para associar o usuário atual a nova chave de ID de armazenamento da Microsoft (a ID de usuário será em bedded na chave). Caso contrário, se você não precisa associar uma ID de usuário com a chave do repositório de ID da Microsoft, você pode passar qualquer valor de cadeia de caracteres para o parâmetro *publisherUserId* .
...
O diagrama a seguir ilustra o processo de criação de uma chave do repositório de ID da Microsoft.

  ![Criar chave de ID de repositório do Windows](images/b2b-1.png)
...
O diagrama a seguir descreve o processo de chamar um método na coleção Microsoft Store API API ou a compra de seu serviço.
  ...
  ![Chamar conjuntos ou puchase API](images/b2b-2.png)
