...
ms.locfileid: "57599111"
---
# <a name="get-app-reviews"></a>Obter avaliações de app


Use este método na API de análise da Microsoft Store para obter dados de opinião em formato JSON para um determinado intervalo de datas e outros filtros opcionais. Essas informações também estão disponíveis na [examina o relatório](../publish/reviews-report.md) no Partner Center.
...
| deviceType  | eq, ne  | Uma das seguintes cadeias de caracteres:<ul><li><strong>PC</strong></li><li><strong>Telefone</strong></li><li><strong>Console</strong></li><li><strong>IoT</strong></li><li><strong>Holographic</strong></li><li><strong>Desconhecido</strong></li></ul>  |
...
| productFamily  | eq, ne  | Uma das seguintes cadeias de caracteres:<ul><li><strong>PC</strong></li><li><strong>Tablet</strong></li><li><strong>Telefone</strong></li><li><strong>Dispositivo portátil</strong></li><li><strong>Servidor</strong></li><li><strong>Colaboração</strong></li><li><strong>Outros</strong></li></ul>  |
...
|------------|--------|------------------|
| Valor      | matriz  | Uma matriz de objetos que contêm dados de opinião. Para saber mais sobre os dados em cada objeto, consulte a seção de [valores de opinião](#review-values) a seguir.       |
...
| Valor           | Tipo    | Descrição       |
|-----------------|---------|-------------------|
| date            | cadeia de caracteres  | A primeira data no intervalo de datas dos dados de análise. Se a solicitação especificou um único dia, o valor será essa data. Se a solicitação especificou uma semana, um mês ou outro intervalo de datas, o valor será a primeira data nesse intervalo de datas. |
| applicationId   | cadeia de caracteres  | A ID da Loja do app para o qual você está recuperando dados de análise.         |
| applicationName | cadeia de caracteres  | O nome de exibição do app.    |
| market          | cadeia de caracteres  | O código do país ISO 3166 do mercado em que a análise foi enviada.        |
| osVersion       | cadeia de caracteres  | A versão do sistema operacional no qual a análise foi enviada. Para obter uma lista das cadeias de caracteres com suporte, consulte a seção [Campos de filtro](#filter-fields) acima.            |
| deviceType      | cadeia de caracteres  | O tipo de dispositivo no qual a análise foi enviada. Para obter uma lista das cadeias de caracteres com suporte, consulte a seção [Campos de filtro](#filter-fields) acima.            |
| isRevised       | Booliano | O valor **true** indica que a opinião foi revisada; caso contrário, **false**.   |
| packageVersion  | cadeia de caracteres  | A versão do pacote do aplicativo que foi revisado.        |
| deviceModel        | cadeia de caracteres  |O tipo de dispositivo no qual o aplicativo foi avaliado.     |
| productFamily      | cadeia de caracteres  | O nome da família de dispositivos. Para obter uma lista das cadeias de caracteres com suporte, consulte a seção [Campos de filtro](#filter-fields) acima.   |
| deviceRAM       | number  | A RAM física, em MB.    |
| deviceScreenResolution       | cadeia de caracteres  | A resolução de tela do dispositivo no formato "*largura* x *altura*".    |
...
| reviewerName | cadeia de caracteres | O nome do revisor. |
| rating | number | A classificação do app, em estrelas. |
| reviewTitle | cadeia de caracteres | O título da análise. |
| reviewText | cadeia de caracteres | O conteúdo de texto da análise. |
...
| responseDate | cadeia de caracteres | A data em que uma resposta foi enviada. |
| responseText | cadeia de caracteres | O conteúdo de texto da resposta. |
| id | cadeia de caracteres | A ID da revisão (este é um GUID). Você pode usar essa ID nos métodos [obter as informações de resposta para avaliações de aplicativo](get-response-info-for-app-reviews.md) e [enviar respostas às críticas do aplicativo](submit-responses-to-app-reviews.md). |
