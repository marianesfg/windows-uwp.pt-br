---
author: mcleanbyron
description: "Use os exemplos de código Java nesta seção para saber mais sobre como enviar opções de jogo e trailers usando a API de envio da Windows Store."
...
ms.prod: windows
ms.technology: uwp
keywords: "Windows 10, uwp, API de envio da Windows Store, exemplos de código, opções de jogo, trailers, listagens avançadas, java"
...
Este artigo fornece exemplos de código Java que demonstram como usar a [API de envio da Windows Store](create-and-manage-submissions-using-windows-store-services.md) para estas tarefas:

* Obter um token de acesso do Azure AD a ser usado com a API de envio da Windows Store.
...
A classe ```CreateAndSubmitSubmissionExample``` implementa um programa ```main``` que chama outros métodos de exemplo para usar a API de envio da Windows Store para criar e confirmar um envio de aplicativo que contém as opções de jogo e um trailer. Para adaptar este código para seu próprio uso:
...
* Atribua a variável ```applicationId``` à [ID da Loja](in-app-purchases-and-trials.md#store_ids) do app para o qual você deseja criar um envio.
...
A classe ```DevCenterAccessTokenClient``` define um método auxiliar que usa os seus valores ```tenantId```, ```clientId``` e ```clientSecret``` para criar um token de acesso Azure AD a ser usado com a API de envio da Windows Store.
...
A classe ```DevCenterClient``` define os métodos auxiliares que invocam uma variedade de métodos na API de envio da Windows Store e carregam o arquivo ZIP que contém os pacotes, as imagens de listagem e os arquivos de trailer para o envio de aplicativo.
