...
Um aplicativo de experiência de demonstração de revenda acompanha um build único que pode ser iniciado em um dos dois modos diferentes _normal_ ou _revenda_. Do ponto de vista dos clientes, existe apenas um aplicativo e, para ajudar os clientes na distinção entre as duas versões, é recomendável para o aplicativo em execução no modo de revenda exibir as palavras "Retail" em destaque na barra de título ou em um local indicado.
