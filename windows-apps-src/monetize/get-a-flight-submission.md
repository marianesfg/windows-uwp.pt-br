...
| flightId | string | Obrigatório. A ID do pacote de pré-lançamento que contém o envio que você deseja obter. Essa ID está disponível nos dados de resposta de solicitações para [criar um pacote de pré-lançamento](create-a-flight.md) e [obter pacotes de pré-lançamento para um aplicativo](get-flights-for-an-app.md).  |
