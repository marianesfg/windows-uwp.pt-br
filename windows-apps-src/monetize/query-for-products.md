...
| localTicketReference | cadeia de caracteres | O identificador solicitado para os produtos retornados. Itens retornados no corpo da resposta terão uma correspondência *localTicketReference*. Recomendamos que você use o mesmo valor do que a declaração *userId* na chave ID da Microsoft Store. | Sim      |
...
| orderId              | cadeia de caracteres             | Se presente, a ID do pedido do qual este item foi obtido.              | Não       |
