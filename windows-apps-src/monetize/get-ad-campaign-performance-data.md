...
Use esse método na API de análise da Windows Store para obter um resumo agregado dos dados de desempenho da campanha publicitária promocional dos aplicativos durante um determinado intervalo de datas e outros filtros opcionais. Este método retorna os dados no formato JSON.
...
Para criar, atualizar ou recuperar detalhes de campanhas publicitárias, você pode usar os métodos de [Gestão de campanhas publicitárias](manage-ad-campaigns.md) na [API de promoções da Windows Store](run-ad-campaigns-using-windows-store-services.md).
