...
Se você preferir usar XML bruto em vez da [Biblioteca de notificações](https://www.nuget.org/packages/Microsoft.Toolkit.Uwp.Notifications/), consulte [o esquema XML](tiles-and-notifications-toast-xml-schema.md).
...
| Propriedade | Tipo | Obrigatório |Descrição |
|---|---|---|---|
| **Texto** | string ou BindableString | false | O texto para exibição. Suporte à vinculação de dados adicionado à Atualização de criadores, mas funciona apenas para elementos de texto de nível superior. |
...
|---|---|---|---|
| **Id** | string | true | A Id é obrigatória e é usada para mapear o texto inserido pelo usuário em um par chave-valor de id/valor que o aplicativo consome posteriormente. |
| **Title** | string | false | O texto do título que será exibido acima da caixa de seleção. |
...
| Propriedade | Tipo | Obrigatório | Descrição |
|---|---|---|---|
| **url** | uri | false | O arquivo de mídia para reprodução em vez do som padrão. Suporte apenas para ms-appx e ms-appdata. |
| **loop** | booleano | false | Defina como true se o som deve repetir enquanto a Notificação do sistema for exibida; false para reproduzir apenas uma vez (padrão). |
| **silent** | booleano | false | True para desativar o som; false para permitir que o som da notificação do sistema seja reproduzido (padrão). |


## <a name="toastheader"></a>ToastHeader
Um cabeçalho personalizado que agrupa várias notificações na Central de Ações.
...
| **title** | string | true | Um título para o cabeçalho. |
| **action**| Ação | false | Declara qual ação será executada quando a notificação do sistema é clicada pelo usuário. |
