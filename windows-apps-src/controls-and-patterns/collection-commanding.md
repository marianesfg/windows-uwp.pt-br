...
> **APIs importantes**: [interface ICommand](https://docs.microsoft.com/en-us/uwp/api/Windows.UI.Xaml.Input.ICommand), [propriedade UIElement.ContextFlyout](https://docs.microsoft.com/en-us/uwp/api/Windows.UI.Xaml.UIElement#Windows_UI_Xaml_UIElement_ContextFlyout), [interface INotifyPropertyChanged](https://docs.microsoft.com/en-us/uwp/api/windows.ui.xaml.data.inotifypropertychanged)
...
Como os usuários podem interagir com um aplicativo UWP usando [uma ampla gama de dispositivos e entradas](../input-and-devices/device-primer.md), seu aplicativo deve expor comandos por menus de contexto independentes do tipo de entrada e aceleradores de entrada específicos. A inclusão de ambos permite que o usuário invoque rapidamente comandos no conteúdo, independentemente do tipo de entrada ou dispositivo.
...
Observe que PodcastObject implementa [INotifyPropertyChanged](https://docs.microsoft.com/en-us/uwp/api/Windows.UI.Xaml.Data.INotifyPropertyChanged) para responder às alterações de propriedade quando o usuário alterna a propriedade IsFavorite.

## <a name="defining-commands-with-the-icommand-interface"></a>Definição de comandos com a interface ICommand

A [interface ICommand](https://docs.microsoft.com/en-us/uwp/api/Windows.UI.Xaml.Input.ICommand) ajuda você a definir um comando que está disponível para vários tipos de entrada. Por exemplo, em vez de escrever o mesmo código para um comando de exclusão nos dois manipuladores de eventos diferentes, um para quando o usuário pressiona a tecla Delete e outro para quando o usuário clica com o botão direito do mouse em "Excluir" em um menu de contexto, você pode implementar a lógica de excluir uma vez como um [ICommand](https://docs.microsoft.com/en-us/uwp/api/Windows.UI.Xaml.Input.ICommand)e, em seguida, torná-lo disponível para diferentes tipos de entrada.

É necessário definir o ICommand que representa a ação "Favoritos". Vamos usar o método [Execute](https://docs.microsoft.com/en-us/uwp/api/Windows.UI.Xaml.Input.ICommand#Windows_UI_Xaml_Input_ICommand_Execute_) do comando para incluir o podcast nos favoritos. O podcast específico será fornecido para o método execute por meio do parâmetro do comando, que pode ser vinculado usando a propriedade CommandParameter.
...
Para executar o comando, é necessário chamar o método [Execute](https://docs.microsoft.com/en-us/uwp/api/Windows.UI.Xaml.Input.ICommand#Windows_UI_Xaml_Input_ICommand_Execute_).
...
Quando você tiver uma lista de itens e cada um deve responder a várias entradas, é possível simplificar o código ao definir uma [UserControl](https://docs.microsoft.com/en-us/uwp/api/Windows.UI.Xaml.Controls.UserControl) para o item e usá-lo para definir manipuladores de eventos e o menu de contexto dos itens. 
...
Para encapsular esses comportamentos e usar o FavoriteCommand, vamos criar um novo [UserControl](https://docs.microsoft.com/en-us/uwp/api/Windows.UI.Xaml.Controls.UserControl) chamado "PodcastUserControl" para representar um podcast na lista.
...
Observe que você também pode usar o [evento ContextRequested](https://docs.microsoft.com/en-us/uwp/api/Windows.UI.Xaml.UIElement#Windows_UI_Xaml_UIElement_ContextRequested) para responder às ações de contexto. O evento ContextRequested não será acionado se um ContextFlyout foi especificado.
...
O aplicativo pode responder quando o usuário pressionar uma tecla usando o evento [KeyDown](https://docs.microsoft.com/en-us/uwp/api/Windows.UI.Xaml.UIElement#Windows_UI_Xaml_UIElement_KeyDownEvent). Em geral, os usuários esperam que o aplicativo responda ao pressionarem a tecla em vez de aguardar até que a soltem.
...
Os botões de foco devem aparecer e desaparecer quando o mouse entra e sai do item. Para responder a eventos de mouse, você pode usar os eventos [PointerEntered](https://docs.microsoft.com/en-us/uwp/api/Windows.UI.Xaml.UIElement#Windows_UI_Xaml_UIElement_PointerEnteredEvent) e [PointerExited](https://docs.microsoft.com/en-us/uwp/api/Windows.UI.Xaml.UIElement#Windows_UI_Xaml_UIElement_PointerExitedEvent) em PodcastUserControl.
...
Os comandos efetuados ao deslizar o dedo são aceleradores de toque que permitem aos usuários em dispositivos sensíveis ao toque executar ações comuns secundárias. Ao deslizar o dedo, os usuários podem interagir com conteúdo de maneira rápida e natural, usando as ações comuns como passar o dedo para excluir ou passar o dedo para invocar. Consulte o artigo sobre [comandos de deslizar o dedo](http://windowsstyleguide/controls-and-patterns/swipe/) para saber mais.

Para integrar o deslize de dedo em sua coleção, você precisa de dois componentes: um SwipeContent que hospeda os comandos e um SwipeContainer que encapsula o item e possibilita a interação de deslizar o dedo.

O SwipeContent pode ser definido como um recurso em PodcastUserControl. Neste exemplo, o SwipeContent contém um comando para um item Favorito.
...
O SwipeContainer encapsula o item e permite ao usuário interagir com ele usando o gesto de deslizar o dedo. Observe que o SwipeContainer contém uma referência ao SwipeContent como sua RightContent. O item Favoritos será exibido quando o usuário deslizar o dedo da direita para a esquerda.
...
A ação de Puxar para atualizar permite a um usuário extrair uma coleção de dados com toque para recuperar mais dados. Consulte o artigo [puxar para atualizar](http://windowsstyleguide/controls-and-patterns/pull-to-refresh-rs3/) a fim de saber mais.
...
Para otimizar seu aplicativo para entrada de caneta, consulte o artigo [interação com caneta](http://windowsstyleguide/input-and-devices/pen-and-stylus-interactions/).
...
* Use a [interface ICommand](https://docs.microsoft.com/en-us/uwp/api/Windows.UI.Xaml.Input.ICommand) para implementar comandos. 

## <a name="related-topics"></a>Tópicos relacionados
* [Interface ICommand](https://docs.microsoft.com/en-us/uwp/api/Windows.UI.Xaml.Input.ICommand)
* [Menus e menus de contexto](http://windowsstyleguide/controls-and-patterns/menus/)
* [Passar o dedo](http://windowsstyleguide/controls-and-patterns/swipe/)
* [Puxar para atualizar](http://windowsstyleguide/controls-and-patterns/pull-to-refresh)
* [Interação com caneta](http://windowsstyleguide/input-and-devices/pen-and-stylus-interactions/)
* [Adaptar seu aplicativo para gamepad e Xbox](http://windowsstyleguide/input-and-devices/designing-for-tv)
