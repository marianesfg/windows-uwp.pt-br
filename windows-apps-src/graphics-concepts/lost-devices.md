...
Um dispositivo Direct3D pode estar em um estado operacional ou perdido. O estado *operacional* é o normal do dispositivo, no qual ele funciona e apresenta as renderizações conforme o esperado. O dispositivo faz uma transição para o estado *perdido* quando um evento, como a perda de foco do teclado em um app de tela inteira, impossibilita a renderização. O estado perdido é caracterizado pela falha silenciosa de todas as operações de renderização, ou seja, os métodos de renderização podem retornar códigos de sucesso mesmo em caso de falha das operações de renderização.
