...
Regras de rasterização definem como os dados de vetor são mapeados para dados de rasterização. Os dados de rasterização são ajustados para locais de inteiro que são, em seguida, selecionados e recortados (para desenhar o número mínimo de pixels) e atributos por pixel são interpolados (de atributos por vértice) antes de serem passados para um sombreador de pixel.
