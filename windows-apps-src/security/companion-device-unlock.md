...
description: Dispositivo complementar do Windows Hello é um dispositivo que pode atuar em conjunto com sua área de trabalho do Windows 10 para melhorar a experiência de autenticação do usuário. Usando a estrutura de dispositivos do Windows Hello Companion, um dispositivo complementar pode fornecer uma experiência rica para o Windows Hello, mesmo quando a biometria não estiver disponível (por exemplo, se a área de trabalho do Windows 10 estiver sem uma câmera para autenticação facial ou dispositivo de leitor de impressão digital , por exemplo).
ms.date: 02/08/2017
ms.topic: article
...
> **Observação** A Microsoft estará preterindo a API para a estrutura de dispositivo do Windows Hello Companion no primeiro semestre de 2020.
...
Se o dispositivo complementar for compatível com biometria, em alguns casos a [Windows Biometric Framework](https://msdn.microsoft.com/windows/hardware/commercialize/design/device-experiences/windows-hello) pode ser uma solução melhor do que a estrutura de dispositivo complementar do Windows Hello. Entre em contato com [cdfonboard@microsoft.com](mailto:cdfonboard@microsoft.com) e ajudaremos você a escolher a abordagem correta.
...
- **Sinal de intenção**: Permite que o usuário mostre sua intenção de desbloquear por, por exemplo, ao pressionar um botão no dispositivo do Windows Hello Companion. O sinal de intenção deve ser coletado no lado do **dispositivo complementar do Windows Hello**.
...
- **Sinal de desambiguidade**: Desambigüidades que o usuário deseja desbloquear na área de trabalho do Windows quando várias opções estão disponíveis para o dispositivo do Windows Hello Companion.
...
- Recomendamos que a descoberta, o download e o lançamento do aplicativo do dispositivo Windows Hello Companion sejam simplificados e, se possível, automatizados (por exemplo, o aplicativo pode ser baixado ao tocar no dispositivo Windows Hello Companion em um leitor de NFC no dispositivo Windows 10 desktop lado). No entanto, isso é responsabilidade do dispositivo complementar do Windows Hello e do aplicativo de dispositivo complementar do Windows Hello.
...
A experiência no lado do dispositivo complementar do Windows Hello é mantida e gerenciada pelo aplicativo de dispositivo complementar do Windows Hello. A estrutura de dispositivo complementar do Windows Hello não tem controle sobre essa parte da experiência do usuário. Mais especificamente, o provedor de autenticação complementar informa o aplicativo de dispositivo do Windows Hello Companion (por meio de seu aplicativo em segundo plano) sobre alterações de estado na interface do usuário de logon (por exemplo, a tela de bloqueio acabou de ficar inativa ou o usuário acabou de escrever a tela de bloqueio pressionando a barra de espaços), e é responsabilidade do aplicativo de dispositivo do Windows Hello Companion criar uma experiência em relação a isso (por exemplo, na barra de espaço de pressionamento do usuário e na tela de desbloqueio de ortografia, começar a procurar o dispositivo por USB).
...
- Um serviço de filtro para Enterprise: Uma empresa pode limitar o conjunto de dispositivos complementares do Windows Hello que podem funcionar em seu ambiente para selecionar alguns de um fornecedor específico. Por exemplo, a empresa Contoso pode encomendar do Fornecedor X 10.000 dispositivos complementares do Modelo Y e garantir que apenas esses dispositivos funcionarão no domínio da Contoso (e não em qualquer outro modelo de dispositivo do Fornecedor X).
- Levantamento  Uma empresa pode determinar a lista de dispositivos complementares existentes usados em um ambiente empresarial.
- Revogação em tempo real: Se um funcionário relatar que seu dispositivo complementar é perdido ou roubado, o serviço Web pode ser usado para revogar esse dispositivo.
- Roaming Um usuário só precisa registrar seu dispositivo complementar uma vez e ele funciona em todos os seus desktops Windows 10 e dispositivos móveis.
...
3. Aguarde o evento certo na tarefa em segundo plano
    * WaitingForUserConfirmation: Aguarde esse evento se a ação/gesto do usuário no lado do dispositivo Windows Hello Companion for necessário para iniciar o fluxo de autenticação
    * CollectingCredential: Aguarde esse evento se o dispositivo complementar do Windows Hello depender de ação/gesto do usuário no lado do computador para iniciar o fluxo de autenticação (por exemplo, pressionando a barra de espaços)
...
O registro requer duas chamadas à API para o serviço de autenticação complementar: RequestStartRegisteringDeviceAsync e FinishRegisteringDeviceAsync.
...
O aplicativo de dispositivo complementar do Windows Hello pode chamar AbortRegisteringDeviceAsync para cancelar o registro e transmitir um código de erro. O Serviço de Autenticação Complementar registrará o erro nos dados de telemetria. Um bom exemplo para essa chamada seria quando algo deu errado com o dispositivo Windows Hello Companion e não pôde concluir o registro (por exemplo, ele não pode armazenar chaves HMAC ou a conexão BT foi perdida).

O aplicativo de dispositivo do Windows Hello Companion deve fornecer uma opção para que o usuário Desregistre seu dispositivo complementar do Windows Hello de sua área de trabalho do Windows 10 (por exemplo, se ele perdeu seu dispositivo complementar ou comprou uma versão mais recente). Quando o usuário selecionar essa opção, o aplicativo de dispositivo complementar do Windows Hello deverá chamar UnregisterDeviceAsync. Essa chamada pelo aplicativo de dispositivo complementar do Windows Hello disparará o serviço de autenticação de dispositivo complementar de forma a excluir todos os dados (incluindo chaves HMAC) correspondentes à ID de dispositivo específica e ao AppId do aplicativo de chamada no lado do computador. Essa chamada de API não tenta excluir chaves HMAC no lado do aplicativo de dispositivo complementar do Windows Hello ou no lado do dispositivo complementar. Isso é deixado para o aplicativo de dispositivo complementar do Windows Hello implementar.
...
A autenticação requer duas chamadas à API para o serviço de autenticação complementar: StartAuthenticationAsync e FinishAuthencationAsync.
...
| Estado                         | Descrição                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
|----------------------------   |-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------    |
| WaitingForUserConfirmation    | Esse evento de notificação de alteração de estado é acionado quando a tela de bloqueio é inativa (por exemplo, o usuário pressionou Windows + L). Convém não solicitar mensagens de erro relacionadas a dificuldades de encontrar um dispositivo nesse estado. Em geral, convém apenas mostrar apenas mensagens quando o sinal de intenção está disponível. O aplicativo do dispositivo Windows Hello Companion deve fazer a primeira chamada à API para autenticação nesse estado se o dispositivo complementar coletar o sinal de intenção (por exemplo, tocando no leitor NFC, pressione um botão no dispositivo complementar ou um gesto específico, como Clapping ), e a tarefa de segundo plano do aplicativo de dispositivo do Windows Hello recebe a indicação do dispositivo complementar que o sinal de intenção foi detectado. Caso contrário, se o aplicativo de dispositivo complementar do Windows Hello depender do computador para iniciar o fluxo de autenticação (fazendo com que o usuário passe o dedo para cima na tela de desbloqueio ou pressione a barra de espaço), ele precisará aguardar o próximo estado (CollectingCredential).   |
| CollectingCredential          | Esse evento de notificação de mudança de estado é disparado quando o usuário abre a tampa do notebook, pressiona qualquer tecla no teclado ou passe o dedo para cima na tela de desbloqueio. Se o dispositivo complementar do Windows Hello depender das ações acima para começar a coletar o sinal de intenção, o aplicativo de dispositivo complementar do Windows Hello deverá começar a coletá-lo (por exemplo, por meio de um pop-up no dispositivo que está perguntando se o usuário deseja desbloquear o PC) . Isso seria um bom momento para fornecer casos de erro se o aplicativo de dispositivo complementar do Windows Hello precisar que o usuário forneça um sinal de presença no dispositivo complementar (como digitar um PIN nesse dispositivo).                                                                                                                                                                                                                                                                                                                                             |
