...
As marcas são exatamente iguais (correspondência de todos os elementos de submarca). Uma comparação pode ser promovida para esse tipo de correspondência com base em uma correspondência de região ou variante.
...
As marcas correspondem às submarcas de idioma, script e região, mas diferem em algum outro aspecto.

### <a name="partial-matches"></a>Correspondências parciais

As marcas correspondem às submarcas de idioma e script, mas diferem na região ou em alguma outra submarca.
...
As marcas correspondem às submarcas de idioma e script, e uma das submarcas de região é a submarca de região padrão do idioma. Por exemplo, "fr-FR" é a região padrão da submarca "fr". Isso depende dos dados mantidos no Windows que definem uma região padrão para cada idioma em que o Windows é localizado.
...
As submarcas de idioma principais incompatíveis serão classificadas abaixo do nível de uma correspondência válida.
...
Quando uma submarca de idioma tem um valor de supressão de script definido no Registro de submarca BCP-47, a respectiva correspondência ocorre, assumindo o valor do código de script suprimido. No exemplo a seguir, o idioma do usuário é "en-AU" (Inglês (Austrália)).
...
Esses idiomas com variantes regionais significativas apresentam alguns desafios à criação de um app para uso internacional: "Quantas variantes regionais diferentes devem ser compatíveis?" "Quais?" "Qual é a maneira mais econômica de gerenciar esses ativos de variante regional para meu app?" It's beyond the scope of this topic to answer all these questions. No entanto, os mecanismos de correspondência de idioma no Windows fornecem recursos que podem ajudar você a tratar variantes regionais.
