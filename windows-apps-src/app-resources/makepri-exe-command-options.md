...
Você pode executar `MakePri.exe help` ou `MakePri.exe /?` para ver os comandos que podem ser usados com o MakePri. exe. Você também pode emitir `MakePri.exe <command> /?` para ver informações específicas sobre um comando e, em casos muito raros, até mesmo `MakePri.exe <command> <option>` para ver informações específicas sobre uma opção.
...
Use a opção de opções de índice (/Io) com `new`, `resourcepack`e `versioned` para especificar opções que fornecem controle detalhado sobre o comportamento dos indexadores de recursos. As opções de índice são desabilitadas por padrão.
...
Se os pacotes de recursos forem criados, o resumo de saída de MakePRI.exe será mais detalhado. Veja um exemplo.
