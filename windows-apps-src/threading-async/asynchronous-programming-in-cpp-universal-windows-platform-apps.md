...
Embora você pode consumir as APIs de UWP assíncronas diretamente no C++, a abordagem preferida é usar o [ **classe task** ][task-class] and its related types and functions, which are contained in the [**concurrency**][concurrencyNamespace] namespace e definido no `<ppltasks.h>`. A **concurrency::task** é um tipo de finalidade geral, mas quando é usado o switch do compilador **/ZW**, que é obrigatório para aplicativos e componentes Plataforma Universal do Windows (UWP), a classe task encapsula os tipos assíncronos UWP para que seja mais fácil:
...
Este artigo fornece orientação básica sobre como usar a classe **task** com as APIS assíncronas UWP. Para obter mais documentação completa sobre **tarefa** e seus métodos relacionados incluindo [ **crie\_tarefa**][createTask], see [Task Parallelism (Concurrency Runtime)][taskParallelism]. 
...
A tarefa que tenha criado e retornado pelo [ **Task:: Then** ][taskThen] função é conhecida como um *continuação*. O argumento de entrada (neste caso) para o lambda fornecido pelo usuário é o resultado que a operação da tarefa produz quando concluída. É o mesmo valor que seria recuperado chamando [**IAsyncOperation::GetResults**](https://docs.microsoft.com/uwp/api/Windows.Foundation.IAsyncOperation_TResult_#Windows_Foundation_IAsyncOperation_1_GetResults) se você estivesse usando diretamente a interface **IAsyncOperation**.

O [ **Task:: Then** ][taskThen] método retorna imediatamente e seu delegado não será executado até que o trabalho assíncrono seja concluído com êxito. Neste exemplo, se a operação assíncrona causar o lançamento de uma exceção ou terminar no estado cancelado como resultado de uma solicitação de cancelamento, a continuação jamais será executada. Posteriormente, descreveremos como gravar continuações que são executadas mesmo quando a tarefa anterior é cancelada ou apresenta falha.
...
-   A primeira continuação converte o objeto [**IAsyncAction^** ][IAsyncAction] em uma **task<void>** e retorna a **tarefa**.
...
-   A segunda continuação não é executado até que o [ **DeleteAsync** ][deleteAsync] operação é concluída.

-   Como a segunda continuação é baseada em valor, se a operação foi iniciada pela chamada para [ **DeleteAsync** ][deleteAsync] gera uma exceção, a segunda continuação não é executado em todos os.
...
Em uma continuação baseada em tarefa, chamamos a função de membro [ **Task:: Get** ][taskGet] para obter os resultados da tarefa. Ainda é necessário chamar **task::get** mesmo que a operação seja uma [**IAsyncAction**][IAsyncAction] que não produz resultados porque **task::get** também obtém todas as exceções que foram transportadas adiante para a tarefa. Se a tarefa de entrada armazena uma exceção, ela será lançada na chamada de **task::get**. Se você não chamar **Task:: Get**, não use uma continuação baseada em tarefa no final da cadeia ou não captura o tipo de exceção foi lançado, então um **observada\_tarefa\_exceção** é lançada quando todas as referências para a tarefa tem sido excluídas.
...
A interface do usuário de um aplicativo UWP é executada em um STA. Uma tarefa cujo lambda retorna um [ **IAsyncAction**][IAsyncAction] or [**IAsyncOperation**][IAsyncOperation] reconhecimento de compartimento. Se a tarefa for criada no STA, todas as suas continuações também serão executadas nele por padrão, a menos que você especifique de outra forma. Em outras palavras, a cadeia de tarefas inteira herda o reconhecimento de apartment da tarefa pai. Esse comportamento ajuda a simplificar as interações com os controles da interface do usuário, que podem ser acessados somente pelo STA.

Por exemplo, em um aplicativo UWP, na função de membro de qualquer classe que representa uma página XAML, você pode preencher uma [ **ListBox** ](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.ListBox) controle de dentro um [ **Task:: Then** ][taskThen] método sem ter que usar o [ **Dispatcher** ](https://docs.microsoft.com/uwp/api/Windows.UI.Core.CoreDispatcher) objeto.
...
Se uma tarefa não retornar um [ **IAsyncAction**][IAsyncAction] or [**IAsyncOperation**][IAsyncOperation], em seguida, ele não está ciente de apartment e, por padrão, suas continuações são executadas na primeira thread de plano de fundo disponíveis.
