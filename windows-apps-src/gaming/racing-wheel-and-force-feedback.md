...
* como volantes de corrida se comportam como um dispositivo de navegação
...
Os volantes de corrida são gerenciados pelo sistema. Portanto, você não precisa criar ou inicializá-los. O sistema fornece uma lista de volantes de corrida conectados e eventos para notificar você quando um volante de corrida é adicionado ou removido.

### <a name="the-racing-wheels-list"></a>A lista de volantes de corrida

A classe [RacingWheel][] fornece uma propriedade estática, [RacingWheels][], que é uma lista somente leitura de volantes de corrida conectados no momento. Como você pode ter interesse apenas em alguns dos volantes de corrida conectados, é recomendável manter a própria coleção em vez de acessá-los por meio da propriedade `RacingWheels`.

O exemplo a seguir copia todos os volantes de corrida conectados para uma nova coleção.
```cpp
...
### <a name="adding-and-removing-racing-wheels"></a>Como adicionar e remover volantes de corrida

Quando um volante de corrida é adicionado ou removido, os eventos [RacingWheelAdded][] e [RacingWheelRemoved][] são acionados. É possível registrar manipuladores para esses eventos a fim de acompanhar os volantes de corrida conectados no momento.

O exemplo a seguir inicia o acompanhamento de um volante de corrida que foi adicionado.
```cpp
...
O exemplo a seguir para o acompanhamento de um volante de corrida que foi removido.
```cpp
...
### <a name="users-and-headsets"></a>Usuários e fones de ouvido

Cada volante de corrida pode ser associado a uma conta de usuário para vincular a identidade ao jogo e pode ter um fone de ouvido conectado para facilitar o chat de voz ou recursos no jogo. Para saber mais sobre como trabalhar com usuários e fones de ouvido, consulte [Como rastrear usuários e os dispositivos](input-practices-for-games.md#tracking-users-and-their-devices) e [Fone de ouvido](headset.md).

## <a name="reading-the-racing-wheel"></a>Como ler o volante de corrida

Depois de identificar os volantes de corrida de interesse, você estará pronto para coletar informações dele. No entanto, diferentemente de outros tipos de entrada com os quais você possa estar acostumado, os volantes de corrida não informam a alteração de estado acionando eventos. Em vez disso, você faz leituras regulares do estado atual deles realizando uma _sondagem_.
...
Às vezes, convém determinar quando um botão passa de pressionado para liberado ou vice-versa, se vários botões foram pressionados ou liberados ou se um conjunto de botões está disposto de determinada maneira – alguns pressionados, outros, não. Para obter informações sobre como detectar essas condições, consulte [Como detectar transições de botão](input-practices-for-games.md#detecting-button-transitions) e [Como detectar disposições de botão complexas](input-practices-for-games.md#detecting-complex-button-arrangements).
...
> **Observação**    A borboleta de câmbio, quando compatível, fica ao lado dos botões de marcha anterior e posterior obrigatórios, o que também afeta a marcha atual do carro do jogador. Uma estratégia simple para unificar essas entradas nas quais ambos estejam presentes é ignorar a borboleta de câmbio (e a embreagem) quando um jogador escolhe uma transmissão automática para o carro e só ignorar os botões de marcha anterior e posterior quando um jogador escolhe uma transmissão manual para o carro caso o volante de corrida esteja equipado com uma borboleta de câmbio. Será possível implementar uma estratégia de unificação diferente se ela não for indicada para o jogo.
