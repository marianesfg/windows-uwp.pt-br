...
Se você estiver usando a compilação condicional (com C# diretivas de pré-processador) para que seus arquivos de código funcionam no Windows 8.1 e Windows Phone 8.1, em seguida, agora você pode examinar essa compilação condicional levando em consideração o trabalho de convergência feito no Windows 10. Convergência significa que, em seu aplicativo do Windows 10, algumas condições podem ser totalmente removidas. Outras mudam para verificações em tempo de execução, conforme demonstrado nos exemplos abaixo.

**Observação**    se você deseja dar suporte a Windows 8.1, Windows Phone 8.1 e Windows 10 em um arquivo de código único, você poderá fazê-lo. Se você olhar no seu projeto do Windows 10 em páginas de propriedades de projeto, você verá que o projeto define WINDOWS\_UAP como um símbolo de compilação condicional. Dessa forma, você pode usá-lo em combinação com o WINDOWS\_aplicativo e do WINDOWS\_PHONE\_aplicativo. Esses exemplos mostram o caso mais simples de remover a compilação condicional de um aplicativo Universal 8.1 e substituindo o código equivalente para um aplicativo do Windows 10.
