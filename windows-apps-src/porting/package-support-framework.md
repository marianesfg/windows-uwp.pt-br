...
Você pode usar o [Arquivo de correção de redirecionamento](https://github.com/Microsoft/MSIX-PackageSupportFramework/tree/master/fixups/FileRedirectionFixup) para redirecionar tentativas de gravação ou leitura de dados em um diretório que não está acessível de um aplicativo que é executado em um contêiner de MSIX.
...
| applications | executável | O caminho relativo de pacote para o executável que você deseja iniciar. Na maioria dos casos, você pode obter esse valor de seu arquivo de manifesto do pacote antes de modificá-lo. Ele é o valor do `Executable` atributo do `Application` elemento. |
...
| correções | DLL | Caminho relativo ao pacote de correção,.msix/.appx para carregar. |
...
Nas páginas de propriedades, localize o campo de **Idioma de C++ padrão** e, em seguida, na lista suspensa ao lado de campo, selecione o **ISO C++ 17 padrão (/ /std: + + 17)** opção.
...
Abra as páginas de propriedades para o projeto e, na página de configurações **gerais** , defina a propriedade de **Nome de destino** para ``PSFLauncher32`` ou ``PSFLauncher64`` dependendo da arquitetura de seu aplicativo.
...
>[!NOTE]
> Se você não tiver o código-fonte para o seu aplicativo, basta escolha o projeto de iniciador PSF. Mostraremos a você como fazer referência a seu executável quando você cria um arquivo de configuração.
...
Declare o ``FIXUP_DEFINE_EXPORTS`` macro e, em seguida, adicione uma instrução para o `fixup_framework.h` na parte superior de cada um. Arquivo CPP em que você pretende adicionar as funções de sua correção de tempo de execução.
...
Primeiro, depuração F5 executa o aplicativo a implantação de arquivos soltos do caminho de pasta de layout do pacote, em vez de instalando a partir de um .msix / pacote. AppX.  A pasta de layout normalmente não tem as mesmas restrições de segurança como uma pasta de pacote instalado. Como resultado, talvez não seja possível reproduzir erros de negação de acesso de caminho de pacote antes da aplicação de uma correção de tempo de execução.
