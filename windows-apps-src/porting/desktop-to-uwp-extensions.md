...
title: "Extensões de app da ponte de Desktop para UWP"
ms.author: normesta
ms.date: 02/08/2017
ms.topic: article
ms.prod: windows
...
## <a name="manifest-xml-namespaces"></a>Namespaces XML de manifesto

As extensões de aplicativos para a Ponte de Desktop exigem vários namespaces XML diferentes. Esses namespaces deve ser definidos no elemento `<Package>` na raiz do seu arquivo de manifesto, desta forma:
...
Se o arquivo de manifesto estiver emitindo erros de compilador, verifique se você está incluindo todos que os namespaces necessários e se os elementos filho XML estão prefixados com o namespace correto. Você também pode consultar um arquivo de manifesto completo e funcional no [repositório de amostras de Ponte de Desktop no GitHub](https://github.com/Microsoft/DesktopBridgeToUWP-Samples).
...
- *StartupTask TaskId* é um identificador exclusivo para a sua tarefa. Usando esse identificador, seu app pode chamar as APIs na classe [**Windows.ApplicationModel.StartupTask**](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.StartupTask) para habilitar ou desabilitar uma tarefa de inicialização de forma programática.
...
## <a name="app-execution-alias"></a>Alias de execução do aplicativo
...
## <a name="protocol-associations"></a>Associações de protocolo
...
## <a name="files-and-file-explorer-integration"></a>Arquivos e integração com o Explorador de Arquivos
...
### <a name="file-context-menu-verbs"></a>Verbos de menu de contexto do arquivo
...
### <a name="shell-context-menu-verbs"></a>Verbos de menu de contexto do shell

A adição de itens ao menu de contexto da pasta do shell não é compatível no momento.

### <a name="multiple-selection-model"></a>Modelo de seleção múltipla
