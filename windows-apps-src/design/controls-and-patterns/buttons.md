...
Use um **SplitButton** quando quiser que o usuário seja capaz de iniciar uma ação imediata ou escolher dentre opções adicionais de forma independente.
...
Botão suspenso herda o evento Click, mas você normalmente não usá-lo. Em vez disso, use a propriedade de submenu para anexar um submenu e invocar ações usando opções de menu no menu suspenso. O submenu é aberta automaticamente quando o botão é clicado.
...
> [!NOTE]
> Quando invocado com toque, o botão de divisão se comporta como um botão; suspenso as duas metades do botão invocar o submenu. Com outros métodos de entrada, um usuário pode invocar metade do botão separadamente.
...
- Quando suspenso estiver aberto, invocação de identificador dos itens a lista suspensa para ambas as alteração qual opção é selecionado e, em seguida, invocá-la. É importante invocar o item de submenu porque o evento Click não ocorre usando o recurso touch do botão.
