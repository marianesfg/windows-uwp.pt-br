...
O submenu de barra de comando tem dois modos de exibição: *recolhido* e *expandido*.
...
O TextCommandBarFlyout exibe comandos de texto em controles TextBlock, TextBox, RichEditBox, RichTextBlock e PasswordBox. Os comandos são automaticamente configurados adequadamente para a seleção de texto atual. Use um CommandBarFlyout para substituir os comandos de texto padrão em controles de texto.
...
Em invocação reativa comandos são mostrados em resposta a uma ação explícita do usuário para solicitar os comandos; Por exemplo, um botão direito do mouse. Isso corresponde ao conceito de tradicional de um [menu de contexto](menus.md).
...
Por padrão, os itens da barra de comandos são adicionados à coleção **PrimaryCommands**. Esses comandos são mostrados na barra de comandos e ficam visíveis em modos expandidos e recolhidos. Ao contrário de CommandBar, comandos principais não automaticamente estouro aos comandos secundários e podem ser truncados.
...
> [!TIP]
> Por padrão, elementos projetados para tela do aplicativo não podem parecer direita em uma barra de comandos. Quando você adiciona um elemento usando AppBarElementContainer, há algumas etapas que você deve seguir para tornar o elemento coincidir com outros elementos da barra de comando:
>
...
> _Um submenu de barra de comandos expandido com zoom personalizado da interface do usuário no menu_
