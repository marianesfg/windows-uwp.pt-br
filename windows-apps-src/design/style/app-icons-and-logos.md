...
:::row:::
    :::column:::
        * A barra de título da janela do aplicativo * a lista de aplicativo no menu Iniciar * o Gerenciador de barra de tarefas e tarefa * blocos do seu aplicativo * tela inicial do aplicativo * na loja Microsoft :::column-end:::
    :::column:::
...
\ * Usado, a menos que você optar por [Exibir carregados apenas imagens na loja](/windows/uwp/publish/app-screenshots-and-images#display-only-uploaded-logo-images-in-the-store). 
...
:::row:::
    :::column:::
        Windows define automaticamente o fator de escala para cada monitor com base em seu DPI (pontos por polegada) e a distância de exibição do dispositivo. 
...
Como ativos de ícone do aplicativo são bitmaps e bitmaps corrigidos sem escala bem, é recomendável fornecer cada ativo de ícone de uma versão para cada fator de escala: 100%, 125%, 150%, 200% e 400%. Isso é muito ícones! Fortunatly, Visual Studio fornece uma ferramenta que facilita a gerar e atualizar esses ícones. 
...
Por padrão, usamos algumas das imagens de seus pacotes na loja, conforme descrito na tabela na parte superior desta página (juntamente com outras [imagens que você fornecer durante o processo de envio](https://docs.microsoft.com/en-us/windows/uwp/publish/app-screenshots-and-images)). No entanto, você tem a opção de impedir que a loja usando as imagens de logotipo nos pacotes do aplicativo ao exibir a listagem de clientes no Windows 10 (incluindo o Xbox) e, em vez disso, tenha a Store use apenas as imagens enviadas. Isso oferece mais controle sobre a aparência do aplicativo em várias telas em toda a loja. (Observe que, se seu produto oferece suporte a versões anteriores do sistema operacional, os clientes ainda poderão ver imagens dos seus pacotes, mesmo se você usar essa opção). Você pode fazer isso na seção **logotipos da loja** da **listagem da Store** etapa do processo de envio.
...
:::row:::
    :::column:::
        2. no **Gerenciador de soluções**, clique duas vezes o arquivo Package.appmxanifest.
    :::column-end:::
    :::column:::
...
:::row:::
    :::column:::
            Visual Studio exibe o Designer de manifesto.
    :::column-end:::
    :::column:::
...
:::row:::
    :::column:::
        3. Clique na guia **Ativos visuais** . :::column-end:::
    :::column:::
...
Tudo que você precisa fazer é forneça uma única imagem e Visaul Studio irá gerar o bloco pequeno, bloco médio, grande bloco, bloco largo, bloco grande, ícone do aplicativo, tela inicial e ativos de logotipo para cada fator de escala do pacote.

Para gerar todos os ativos ao mesmo tempo:
1. Clique o Stores **** ao lado do campo de **origem** e selecione a imagem que você deseja usar. Se você estiver usando uma imagem de bitmap, verifique se é pelo menos 400 por 400 pixels para que você obterá resultados nítidos. Imagens com base em vetor funcionam melhor; O Visual Studio permite que você use AI (Adobe Illustrator) e arquivos PDF. 
...
Square150x150Logo. Scale-escala-100.png, Square150x150Logo de escala de 200.png, Square150x150Logo de escala de 400.png

Observe que o Visual Studio não gera um logotipo de selo por padrão. Isso ocorre porque o logotipo de selo é exclusivo e provavelmente não deve coincidir com seus outros ícones de aplicativo. Para obter mais informações, consulte as [notificações de selo para o artigo de aplicativos UWP](/windows/uwp/design/shell/tiles-and-notifications/badges). 
...
O ativo de ícone do aplicativo aparece em muitos lugares: na barra de tarefas do Windows, o modo de exibição de tarefa, ALT + TAB e o canto inferior direito dos blocos em Iniciar. Como os ativos de ícone do aplicativo aparece em tantos locais, ele tem alguns dimensionamento adicional e plating opções os outros ativos não tem: ativos "tamanho" e "sem fundo" ativos. 

### <a name="target-size-app-icon-assets"></a>Ativos de ícone do aplicativo de tamanho
Além dos tamanhos fator de escala padrão ("Square44x44Logo.scale-400.png"), também recomendamos criar ativos "tamanho". Chamamos tamanho esses ativos como alvo tamanhos específicos, como 16 pixels, em vez de fatores de escala específica, como 400. Ativos de tamanho de destino são para superfícies que não usam o sistema escala de 1X:
...
Por padrão, o Windows usa um ativos baseados no destino sobre um fundo colorido por padrão. Se você quiser, você pode fornecer um ativo sem fundo baseados no destino. "Sem fundo" significa que o ativo será exibido em um plano de fundo transparente. Tenha em mente que esses ativos serão exibidas em uma variedade de cores de fundo. 
...
### <a name="target-and-unplated-sizing"></a>Dimensionamento sem fundo e destino
...
Para obter mais informações, consulte as [notificações de selo para o artigo de aplicativos UWP](/windows/uwp/design/shell/tiles-and-notifications/badges).


## <a name="customizing-asset-padding"></a>Personalizando o preenchimento de ativo

Por padrão, o gerador de ativo Visual Studio aplica preenchimento recomendado para qualquer imagem. Se suas imagens já contêm preenchimento ou se desejar imagens sangramento completo que se estender até o final do bloco, você pode desativar esse recurso desmarcando a caixa de seleção **Aplicar recomendado preenchimento** . 

### <a name="tile-padding-recommendations"></a>Recomendações de preenchimento de bloco
Se você deseja fornecer sua própria preenchimento, aqui estão nossas recomendações de blocos. 

Há 4 tamanhos de bloco: pequeno (71 x 71), médio (150 x 150), todo o (310 x 150) e grande (310 x 310). 
...
Se você não quiser seu ícone para se estender até a borda do bloco, você pode usar pixels transparentes em seus ativos para criar o preenchimento. 
