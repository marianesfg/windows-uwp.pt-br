...
[Dar suporte a seu aplicativo com tarefas em segundo plano](https://docs.microsoft.com/windows/uwp/launch-resume/support-your-app-with-background-tasks) contém informações gerais sobre o uso de tarefas em segundo plano para trabalhar quando o aplicativo não está no primeiro plano Mais especificamente, seu código deve seguir etapas especiais para ser notificado quando ele não for o aplicativo em primeiro plano atual e chegarem dados pela rede para ele. Você usou gatilhos de canal de controle para essa finalidade no Windows 8, e eles ainda são suportados no Windows 10. Informações completas sobre o uso de gatilhos de canal de controle estão disponíveis [**aqui**](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.ControlChannelTrigger). Uma nova tecnologia no Windows 10 fornece funcionalidade melhor com sobrecarga mais baixa para alguns cenários, como soquetes de fluxo habilitado para envio por push: o agente de soquete e gatilhos de atividade de soquete.
...
-   [**ConnectAsync** ](https://docs.microsoft.com/uwp/api/windows.networking.sockets.streamsocket.connectasync) - Verifique a conexão inicial a um serviço de rede e negociar imediatamente para usar SSL/TLS para todas as comunicações.
-   [**UpgradeToSslAsync** ](https://docs.microsoft.com/uwp/api/windows.networking.sockets.streamsocket.upgradetosslasync) -conectar inicialmente a um serviço de rede sem criptografia. O aplicativo pode enviar ou receber dados. Feito isso, atualize a conexão para usar SSL/TLS em todas as comunicações adicionais.
...
O SocketProtectionLevel especifica o nível de proteção de soquete desejado que o aplicativo deseja estabelecer ou para atualizar a conexão. No entanto, o nível de proteção eventual da conexão estabelecida é determinado em um processo de negociação entre ambos os pontos de extremidade da conexão. O resultado pode ser um nível de proteção menor do que o especificado, se o outro ponto de extremidade solicita um nível inferior. 
 ...
 Depois que a operação assíncrona for concluída com sucesso, é possível recuperar o nível de proteção solicitado usado na chamada [**ConnectAsync**](https://docs.microsoft.com/uwp/api/windows.networking.sockets.streamsocket.connectasync) ou [**UpgradeToSslAsync**](https://docs.microsoft.com/uwp/api/windows.networking.sockets.streamsocket.upgradetosslasync) pela propriedade [**StreamSocketinformation.ProtectionLevel**](https://docs.microsoft.com/uwp/api/windows.networking.sockets.streamsocketinformation.protectionlevel). No entanto, isso não reflete que o nível de proteção real que a conexão está usando.
...
[**ConnectAsync** ](https://docs.microsoft.com/uwp/api/windows.networking.sockets.streamsocket.connectasync) pode ser usado para estabelecer a conexão inicial com um serviço de rede e negociar imediatamente para usar SSL/TLS para todas as comunicações. Há dois métodos **ConnectAsync** que dão suporte a um parâmetro *protectionLevel*:

-   [**ConnectAsync (EndpointPair, SocketProtectionLevel)** ](https://docs.microsoft.com/uwp/api/windows.networking.sockets.streamsocket.connectasync) -inicia uma operação assíncrona em um [ **StreamSocket** ](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.StreamSocket) objeto para se conectar a um destino de rede remota especificado como uma [ **EndpointPair** ](https://docs.microsoft.com/uwp/api/Windows.Networking.EndpointPair) objeto e uma [ **SocketProtectionLevel**](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.SocketProtectionLevel).
-   [**ConnectAsync (nome de host, cadeia de caracteres, SocketProtectionLevel)** ](https://docs.microsoft.com/uwp/api/windows.networking.sockets.streamsocket.connectasync) -inicia uma operação assíncrona em um [ **StreamSocket** ](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.StreamSocket) objeto para se conectar a um destino remoto especificado por um nome de host remoto, um nome de serviço remoto e um [ **SocketProtectionLevel**](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.SocketProtectionLevel).
...
A classe [**Windows.Networking.StreamSocket**](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.StreamSocket) dá suporte ao uso de SSL/TLS para autenticar o servidor o qual o aplicativo se comunica. Em certos casos, o aplicativo também precisa autenticar-se ao servidor usando um certificado de cliente TLS. No Windows 10, você pode fornecer um certificado de cliente sobre o [ **StreamSocket.Control** ](https://docs.microsoft.com/uwp/api/Windows.Networking.Sockets.StreamSocketControl) objeto (isso deve ser definido antes que o handshake TLS é iniciado). Se o servidor solicitar o certificado cliente, o Windows responderá com o certificado fornecido.
...
| **Sindicalização** | [**SyndicationClient(PasswordCredential)** ](https://docs.microsoft.com/uwp/api/windows.web.syndication.syndicationclient.) |
...
* [Aprimoramentos de API de rede no Windows 10](https://blogs.windows.com/buildingapps/2015/07/02/networking-api-improvements-in-windows-10/)
