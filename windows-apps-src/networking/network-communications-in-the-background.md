...
Os apps usam tarefas em segundo plano e dois mecanismos principais para manter as comunicações quando não estão em primeiro plano: agente de soquete e gatilhos de canal de controle. Os apps que usam soquetes para conexões de longa duração podem delegar a propriedade de um soquete para um agente de soquete do sistema quando saem do primeiro plano. O agente ativa o app quando chega tráfego no soquete, transfere a propriedade de volta para o app, e o app processa o tráfego de chegada.

## <a name="performing-short-lived-network-operations-in-background-tasks"></a>Realizando operações de rede de curta duração em tarefas em segundo plano

SocketActivityTrigger e ControlChannelTrigger (abordados mais adiante neste tópico) são projetados para apps que mantêm conexões de rede de longa duração que persistem mesmo quando o app é executado em segundo plano. Aplicativos que exigem interações de rede de curta duração como parte da lógica da tarefa em segundo plano (por exemplo, enviando uma solicitação HTTP) pode chamar diretamente as principais APIs de rede ([**DatagramSocket**](https://msdn.microsoft.com/library/windows/apps/br241319), [**StreamSocket**](https://msdn.microsoft.com/library/windows/apps/br226882) ou [**StreamSocketListener**](https://msdn.microsoft.com/library/windows/apps/br226906)). No entanto, essas tarefas devem ser configuradas de modo que funcionem adequadamente em todas as circunstâncias. Tarefas em segundo plano devem usar a condição [InternetAvailable](https://msdn.microsoft.com/library/windows/apps/windows.applicationmodel.background.systemconditiontype.aspx) com sua tarefa em segundo plano ou usar o sinalizador [IsNetworkRequested](https://msdn.microsoft.com/library/windows/apps/windows.applicationmodel.background.backgroundtaskbuilder.isnetworkrequested.aspx) no registro da tarefa em segundo plano. Isso solicita que a infraestrutura de tarefas em segundo plano acompanhe a rede enquanto a tarefa está em execução, mesmo se o dispositivo entrar em modo de espera conectado.

Se sua tarefa em segundo plano não usa [InternetAvailable](https://msdn.microsoft.com/library/windows/apps/windows.applicationmodel.background.systemconditiontype.aspx) nem [IsNetworkRequested](https://msdn.microsoft.com/library/windows/apps/windows.applicationmodel.background.backgroundtaskbuilder.isnetworkrequested.aspx) conforme descrito aqui, sua tarefa em segundo plano não poderá acessar a rede quando estiver em modo de espera conectado (por exemplo, quando a tela do telefone está desativada).

## <a name="socket-broker-and-the-socketactivitytrigger"></a>Agente de soquete e SocketActivityTrigger

Se o app usa conexões [**DatagramSocket**](https://msdn.microsoft.com/library/windows/apps/br241319), [**StreamSocket**](https://msdn.microsoft.com/library/windows/apps/br226882) ou [**StreamSocketListener**](https://msdn.microsoft.com/library/windows/apps/br226906), você deve usar [**SocketActivityTrigger**](https://msdn.microsoft.com/library/windows/apps/dn806009) e o agente de soquete para ser notificado quando chegar tráfego para seu app enquanto ele não está no primeiro plano.

Para que seu app receba e processe os dados recebidos em um soquete quando o app não está ativo, o app deve executar uma configuração única na inicialização e, em seguida, transferir a propriedade do soquete para o agente de soquete quando ele estiver fazendo a transição para um estado não ativo.
...
```
   -   Se o evento foi gerado por atividade de soquete, crie um DataReader no soquete, carregue o leitor de forma assíncrona e, em seguida, use os dados de acordo com o design do seu app. Observe que você deve retornar a propriedade do soquete para o agente de soquete, para ser notificado de outras atividades do soquete novamente.
...
-   No manifesto do pacote do app para a declaração de tarefa em segundo plano do keep-alive.

A amostra a seguir adiciona uma notificação de gatilho de rede e um gatilho keep-alive sob o elemento &lt;Application&gt; em um manifesto de app.
...
-   É possível que o app precise definir várias propriedades e cabeçalhos no objeto [HttpClient](http://go.microsoft.com/fwlink/p/?linkid=241637) ou [HttpClientHandler](http://go.microsoft.com/fwlink/p/?linkid=241638) no namespace [System.Net.Http](http://go.microsoft.com/fwlink/p/?linkid=227894) antes de enviar a solicitação para o URI específico.
-   Um app talvez precise fazer uma solicitação inicial para testar e configurar o transporte adequadamente antes de criar o transporte [HttpClient](http://go.microsoft.com/fwlink/p/?linkid=241637) para uso com [**ControlChannelTrigger**](https://msdn.microsoft.com/library/windows/apps/hh701032). Depois que o app determina que o transporte pode ser configurado adequadamente, um objeto [HttpClient](http://go.microsoft.com/fwlink/p/?linkid=241637) pode ser configurado como o objeto de transporte usado com o objeto **ControlChannelTrigger**. Esse processo foi desenvolvido para impedir que alguns cenários interrompam a conexão estabelecida por meio do transporte. Usando SSL com um certificado, um app pode exigir a exibição de uma caixa de diálogo para entrada do PIN ou caso haja vários certificados disponíveis para escolha. A autenticação de proxy e a autenticação de servidor podem ser necessárias. Se a autenticação de proxy ou de servidor expirar, a conexão poderá ser fechada. Uma maneira pela qual um app pode lidar com esses problemas de expiração da autenticação é definindo um temporizador. Quando um redirecionamento HTTP é necessário, não é garantido que a segunda conexão possa ser estabelecida de maneira confiável. Uma solicitação de teste inicial garante que o app pode usar a URL redirecionada mais recente antes de usar o objeto [HttpClient](http://go.microsoft.com/fwlink/p/?linkid=241637) como transporte com o objeto **ControlChannelTrigger**.
