...
ms.locfileid: "1690982"
---
# <a name="sockets"></a>Soquetes
Soquetes são uma tecnologia de transferência de dados de baixo nível sobre qual muitos protocolos de rede são implementados. UWP oferece classes de soquete de TCP e UDP para o cliente-servidor ou aplicativos ponto a ponto, se as conexões tiverem vida longa ou se uma conexão estabelecida não for necessária.

Este tópico se concentra em como usar as classes de soquete da Plataforma Universal do Windows (UWP) que estão no namespace [**Windows.Networking.Networking**](/uwp/api/Windows.Networking.Sockets?branch=live). Mas você também pode usar [Windows Sockets 2 (Winsock)](https://msdn.microsoft.com/library/windows/desktop/ms740673) em um aplicativo UWP.

> [!NOTE]
> Como consequência do [isolamento de rede](https://msdn.microsoft.com/library/windows/apps/hh770532.aspx), o Windows não permite estabelecer conexões de soquete (soquetes ou WinSock) entre dois aplicativos UWP em execução no mesmo computador, seja um endereço de loopback local (127.0.0.0) ou especificando explicitamente o endereço IP local. Para obter detalhes sobre mecanismos pelo qual os aplicativos UWP podem se comunicar uns com os outros, consulte [Comunicação entre aplicativos](/windows/uwp/app-to-app/index?branch=live).

## <a name="build-a-basic-tcp-socket-client-and-server"></a>Crie um cliente de soquete TCP básico e servidor
Um soquete TCP (protocolo TCP) fornece transferências de dados de rede em baixo nível em qualquer direção para conexões de longa duração. Os soquetes TCP são o recurso subjacente usado pela maioria dos protocolos de rede disponíveis na Internet. Para demonstrar as operações básicas de TCP, o código de exemplo abaixo mostra um [**StreamSocket**](/uwp/api/Windows.Networking.Sockets.StreamSocket?branch=live) e um [**StreamSocketListener**](/uwp/api/Windows.Networking.Sockets.StreamSocketListener?branch=live) enviando e recebendo dados por TCP para formar um cliente de eco e o servidor.
...
Construa um [**StreamSocketListener**](/uwp/api/Windows.Networking.Sockets.StreamSocketListener?branch=live) e inicie a escuta de conexões TCP de entrada. O evento [**StreamSocketListener.Connectionreceived**](/uwp/api/Windows.Networking.Sockets.StreamSocketListener.ConnectionReceived) é acionado sempre que um cliente estabelece uma conexão com o **StreamSocketListener**.

Também construa um [**StreamSocket**](/uwp/api/Windows.Networking.Sockets.StreamSocket?branch=live), estabeleça uma conexão com o servidor, envie uma solicitação e receba uma resposta.
...
## <a name="references-to-streamsockets-in-c-ppl-continuations"></a>Referências a StreamSockets em continuações do PPL em C++
Um [**StreamSocket**](/uwp/api/Windows.Networking.Sockets.StreamSocket?branch=live) permanece ativo, desde que seja uma leitura/gravação ativa em seu fluxo de entrada/saída (por exemplo o [**StreamSocketListenerConnectionReceivedEventArgs.Socket**](/uwp/api/windows.networking.sockets.streamsocketlistenerconnectionreceivedeventargs.Socket) que você tem acesso em seu manipulador de eventos [**StreamSocketListener.ConnectionReceived**](/uwp/api/Windows.Networking.Sockets.StreamSocketListener.ConnectionReceived)). Ao chamar [**DataReader.LoadAsync**](/uwp/api/windows.storage.streams.datareader.loadasync?branch=live) (ou `ReadAsync/WriteAsync/StoreAsync`), então, ele mantém uma referência para o soquete (por meio do fluxo de entrada do soquete) até que o manipulador de conclusão do **LoadAsync** tenha terminado a execução.

Mas o Padrão PPL (PPL) não agenda continuações de tarefa embutidas por padrão. Em outras palavras, adicionar uma tarefa de continuação (com `task::then()`) não garante que a tarefa de continuação será executada embutida como o manipulador de conclusão.
...
Coloque a marcação XAML no `MainPage.xaml` e, em seguida, coloque a necessidade de código dentro da sua classe `MainPage`.
...
Esta seção demonstra algumas técnicas envios em lote que você pode usar com um [**StreamSocket**](/uwp/api/Windows.Networking.Sockets.StreamSocket?branch=live) ou um [**DatagramSocket **](/uwp/api/Windows.Networking.Sockets.DatagramSocket?branch=live) conectado.
...
Esse primeiro exemplo de uma técnica mais eficiente só é adequado se você estiver usando C#. Alterar `OnNavigatedTo` para chamar `BatchedSendsCSharpOnly`, ao invés de `SendMultipleBuffersInefficiently`.
...
Um erro encontrado em uma operação [**DatagramSocket**](/uwp/api/Windows.Networking.Sockets.DatagramSocket?branch=live), [**StreamSocket**](/uwp/api/Windows.Networking.Sockets.StreamSocket?branch=live) ou [**StreamSocketListener**](/uwp/api/Windows.Networking.Sockets.StreamSocketListener?branch=live) é retornado como um valor **HRESULT**. Você pode passar o valor **HRESULT** para o método [**SocketError.GetStatus**](/uwp/api/windows.networking.sockets.socketerror.getstatus?branch=live) convertê-lo em um valor de enumeração [**SocketErrorStatus**](/uwp/api/Windows.Networking.Sockets.SocketErrorStatus?branch=live).
...
* [Comunicação entre aplicativos](/windows/uwp/app-to-app/index?branch=live)
