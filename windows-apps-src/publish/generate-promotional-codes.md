...
-   Você pode gerar códigos promocionais para qualquer aplicativo ou complemento publicado na Windows Store. Os usuários podem resgatar os códigos em qualquer versão do Windows que seja compatível com seu aplicativo ou complemento.
...
1.  No painel do Centro de Desenvolvimento do Windows, siga um destes procedimentos:
    -   Na página **Visão geral do aplicativo** para o seu aplicativo, localize a seção **Códigos promocionais** e clique em **Gerenciar códigos**.
...
2.  Na página **Pedido de novos códigos promocionais**, insira o seguinte:
    ...
    -   Selecione o tipo de pedido. Você pode optar por gerar um conjunto de códigos promocionais que pode ser usado uma vez ou pode optar por gerar um código promocional que pode ser usado várias vezes.
...
3.  Abra o arquivo TSV no editor de sua escolha. Para a melhor experiência, abra o arquivo TSV em um aplicativo que possa exibir os dados em uma estrutura tabular, como o Microsoft Excel. No entanto, você também pode abrir o arquivo em qualquer editor de texto.
    ...
    -   **Nome do produto**: o nome do aplicativo ou complemento ao qual o código esteja associado.
    ...
    -   **URL resgatável**: a URL que um usuário possa usar para resgatar o código e instalar seu aplicativo ou complemento. A URL tem o seguinte formato:
...
4.  Distribua as URLs resgatáveis aos seus usuários por meio de qualquer formato de comunicação de sua preferência (como email, SMS ou cartões impressos). Recomendamos que sua comunicação inclua o seguinte:
    -   Uma explicação do aplicativo ou complemento ao qual o código promocional se aplica e, opcionalmente, uma descrição do motivo pelo qual o usuário está recebendo o código.
...
-   Para analisar um resumo detalhado de pedidos de códigos promocionais para um aplicativo específico, navegue até a página **Códigos promocionais** do seu aplicativo (expanda **Monetização** e clique em **Códigos promocionais**). Você pode examinar os seguintes detalhes para todos os códigos promocionais atuais e inativos para o aplicativo:
    -   Nome do pedido
    -   Nome do aplicativo ou complemento
    -   Data do pedido
    -   Data de validade
    -   Status
