...
**Venda bruta** para seu aplicativo (de outubro de 2016 - presente) também estão disponíveis neste gráfico, mostrando a quantidade total ganho de vendas do aplicativo (em USD). Observe que esse valor não leva em conta para qualquer reembolsos, reversões, estorno etc.
...
Na exibição de **aplicativo diariamente** , quando o **30d** período é selecionado, você poderá ver marcadores de círculo. Eles representam um aumento significativo ou diminuir um valor determinado que acreditamos que você vai querer saber sobre. A data em que o círculo é exibido representa o final da semana em que detectamos um aumento significativo ou redução em comparação comparada a semana anterior. Para ver mais detalhes sobre o que mudou, passe o mouse sobre o círculo.  
