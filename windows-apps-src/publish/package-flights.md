...
> [!NOTE]
> Se tiver mais de um pacote de pré-lançamento, você precisará atribuir uma classificação para cada um deles. Para obter mais informações, consulte [Adicionar e classificar de pré-lançamento adicionais](#add-and-rank-additional-package-flights) abaixo.
...
> [!IMPORTANT]
> Ao gradualmente pacotes em um pacote de pré-lançamento, as pessoas que não estão incluídas na porcentagem que receberem os novos pacotes receberão os pacotes do envio anterior do pacote de pré-lançamento (a menos que haja uma versão de pré-lançamento com classificação mais alta disponível para elas).
...
Seu envio de versão completa é sempre classificado como o mais baixo (n º 1). Ou seja, as pessoas que não estiverem em nenhum de seus grupos de versão de pré-lançamento podem receber apenas pacotes de seu envio de versão completa por meio da Loja. As pessoas em um grupo de versão de pré-lançamento sempre receberão pacotes do pacote classificado de pré-lançamento disponível para elas (mas nunca o envio de versão completa, já que ele tem a classificação mais baixa). Isso proporciona flexibilidade para determinar como distribuir seus pacotes para as pessoas que podem ser membros de mais de um de seus grupos de versão de pré-lançamento.
