...
> [!IMPORTANT]
> Para pacotes do Windows 10 (UWP), a última seção (quarta) do número de versão é reservada para uso de Store e deve ser deixada como 0 quando você compila seu pacote (embora a Store pode alterar o valor nesta seção). As outras seções devem ser definidas como um número inteiro entre 0 e 65535 (exceto a primeira seção, que não pode ser 0).

Ao escolher um pacote UWP do seu envio publicado, a Microsoft Store sempre usará o pacote mais alto número de versão que é aplicável ao dispositivo do Windows 10 do cliente. Isso proporciona maior flexibilidade e coloca você no controle de quais pacotes serão fornecidos para clientes em tipos específicos de dispositivos. Importante, você pode enviar esses pacotes em qualquer ordem. Você não está limitado a fornecer pacotes com maior número de versão em cada envio subsequente.
...
Quando você fornece vários pacotes UWP que usam o mesmo número de versão, a arquitetura (na ordem de x64, x86, ARM, neutro) será usada para decidir qual é a de classificação mais alta (quando a Store determina qual pacote para fornecer ao dispositivo do cliente). Ao classificar lotes de aplicativos que usem o mesmo número de versão, a classificação de arquitetura mais alta dentro do lote será considerada: um lote de aplicativos que contém um pacote x64 terá uma classificação maior do que aquele que contém somente um pacote x86.

Isso dá uma grande flexibilidade para desenvolver seu aplicativo ao longo do tempo. Você pode carregar e enviar novos pacotes que usam números de versão inferiores para adicionar suporte para dispositivos Windows 10 que você não oferecia suporte anteriormente, você pode adicionar pacotes de versão mais atual que têm dependências mais rígidas para tirar proveito do hardware ou recursos do sistema operacional ou pode adicionar pacotes de versão mais atual que servem como atualizações para alguns ou todos os seus clientes existentes base.
...
### <a name="example-moving-to-a-single-package-over-multiple-submissions"></a>Exemplo: Mover para um único pacote ao longo de vários envios
...
Você pode usar as regras de controle de versão do pacote gradualmente mover seus clientes para um único pacote para a família de dispositivo Universal, durante o envio de um número de atualizações provisórias para determinadas famílias de dispositivos (incluindo aqueles que aproveitam APIs do Windows 10). O exemplo a seguir ilustra como as mesmas regras de maneira consistente são aplicadas ao longo de uma série de envios para o mesmo aplicativo.
...
Se você mantiver as cópias de seus pacotes, você terá a opção de reverter pacote do seu aplicativo na Store para um pacote do Windows 10 anterior se você deve descobrir problemas com uma versão. Essa é uma maneira temporária para limitar o interrupções para seus clientes, enquanto você levar tempo para corrigir o problema.
...
Para pacotes. AppX destinados ao Windows 8 e Windows 8.1, a mesma regra aplica-se por arquitetura: o número de versão do pacote em um novo envio sempre deve ser maior que o pacote publicado pela última vez para a Store para a mesma arquitetura.

Além disso, o número de versão de pacotes do Windows 8.1 deve sempre ser maior que os números de versão de qualquer um dos seus pacotes do Windows 8 para o mesmo aplicativo. Em outras palavras, o número de versão de qualquer pacote do Windows 8 que você enviar deve ser menor do que o número de versão de qualquer pacote do Windows 8.1 que você tenha enviado para o mesmo aplicativo.

> [!NOTE]
> Se seu aplicativo também tem os pacotes do Windows 10, o número de versão dos pacotes do Windows 10 deve ser maior do que aqueles para qualquer um dos seus pacotes do Windows 8, Windows 8.1 e Windows Phone 8.1. Para obter mais informações, consulte [adicionando pacotes para o Windows 10 para um aplicativo publicado anteriormente](guidance-for-app-package-management.md#adding-packages-for-windows-10-to-a-previously-published-app).

Aqui estão alguns exemplos do que acontece no número cenários de atualização de versão diferente para os pacotes destinados ao Windows 8 e Windows 8.1.
