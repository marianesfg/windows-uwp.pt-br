...
- Quaisquer críticas escritas por seus testadores ficarão visíveis para você no [Relatório de críticas](reviews-report.md). No entanto, essas análises não serão publicadas na listagem da Store do seu app, mesmo depois que o envio for movido para **Audiência pública**.  
- Quando você move um app de **Audiência particular** para **Audiência pública**, a **Data do lançamento** mostrada na listagem da Store será a data em que ele foi publicado pela primeira vez para a audiência pública (a menos que você tenha especificado uma [data de lançamento de exibição](set-app-pricing-and-availability.md#display-release-date) diferente).
