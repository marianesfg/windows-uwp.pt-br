...
Quando você [começar o processo de inscrição](http://go.microsoft.com/fwlink/p/?LinkId=615100), precisará escolher entre criar uma conta individual ou uma conta empresarial. As contas individuais são projetadas para desenvolvedores autônomos que trabalham por conta própria. Contas de empresa são projetadas para organizações e empresas. As contas de empresa fornecem acesso a alguns recursos adicionais de apps. Também exigimos uma verificação maior para contas de empresa depois do seu registro para confirmar se você está autorizado a configurar a conta para sua empresa. Essa verificação pode levar desde alguns dias a até várias semanas e normalmente inclui um telefonema para sua empresa. Os dois tipos de conta permitirão que você envie apps, suplementos e serviços.
...
|--------------------|-----------------|
| <ul><li>Não pode usar funcionalidades específicas do app</li><li>Custa aproximadamente US$ 19 dólares (taxa de registro única; o valor exato varia dependendo da moeda do seu país ou região)</li></ul> | <ul><li>Requer verificação via Symantec ou Dun & Bradstreet</li><li>Mais acesso a todas as funcionalidades do app</li><li>Requer que sua empresa seja reconhecida como tal no país ou região onde está localizada</li><li>Custa aproximadamente US$ 99 dólares (taxa de registro única; o valor exato varia dependendo da moeda do seu país ou região)</li></ul> |
...
As contas empresariais são um pouco mais caras, principalmente porque exigem algumas etapas adicionais para garantir que você esteja autorizado a representar sua empresa e configurar a conta. Um dos principais benefícios de ter uma conta empresarial é a capacidade de enviar apps que incluem [Declarações de funcionalidades do app](https://msdn.microsoft.com/library/windows/apps/Mt270968) adicionais.

Você deve ter uma conta empresarial, especificamente para publicar apps que usam as três funcionalidades a seguir:

-   **enterpriseAuthentication**—Usa credenciais do Windows para acessar uma intranet corporativa. Normalmente usado em apps de linha de negócios que se conectam aos servidores em uma empresa. (Você não precisa dessa funcionalidade para comunicação genérica pela internet)
-   **sharedUserCertificates**—Permite que um app acesse certificados de hardware e software, como certificados armazenados em um cartão inteligente. Isso normalmente é usado por apps corporativos ou financeiros que exigem um cartão inteligente para autenticação.
-   **documentsLibrary**—Permite acesso programático aos Documentos do usuário, filtrados para a associação de tipo de arquivo declarada no manifesto de pacote. (Você não precisa declarar essa funcionalidade para acessar os Documentos de um usuário com o [seletor de arquivos](https://msdn.microsoft.com/library/windows/apps/Hh465174)). Observe que apps destinados até o Windows Phone 8.1 não podem usar a funcionalidade documentsLibrary.

> **Observação**  Além disso, você deve contatar o suporte e obter uma aprovação antes de poder enviar apps que declaram a funcionalidade **documentsLibrary**.

O fato de ter uma conta de empresa não garante que os apps usando esses recursos passarão pela certificação. Esses recursos se destinam a cenários específicos, e a maioria dos apps não os declara. Para obter mais informações, consulte [Declarações de funcionalidades do app](https://msdn.microsoft.com/library/windows/apps/Mt270968).

Para contas empresariais, usamos a Symantec ou a Dun & Bradstreet para verificar que você está autorizado a criar uma conta para a empresa que está representando. Este processo pode demorar de alguns dias a algumas semanas e geralmente inclui uma chamada telefônica para sua empresa (especifique informações de contato atualizadas ao preencher os formulários de registro). Não é possível enviar apps de uma conta corporativa até que eles tenham sido verificados, mas enquanto está esperando, você pode [reservar um nome de app](https://msdn.microsoft.com/library/windows/apps/JJ657967), acessar seu painel do Centro de Desenvolvimento do Windows, e continuar compilando e testando seu app.

## <a name="developer-account-and-app-submission-markets"></a>Mercados de conta de desenvolvedor e de envio de apps

Você pode se registrar para obter uma conta de desenvolvedor e enviar apps se morar ou tiver uma empresa em um país ou região listado a seguir.
...
A coluna **Pagamento** descreve como pagaremos a você o dinheiro que você ganha nos seus apps. Você poderá escolher a [conta de pagamento](setting-up-your-payout-account-and-tax-forms.md) em que depositaremos o dinheiro.

> **Observação**  Alguns mercados não dão suporte a apps pagos (**pagamento**). Apenas apps grátis podem ser enviados para esses mercados.

A coluna **PayPal** indica se o PayPal é um método de pagamento com suporte que pode ser usado para sua [conta de pagamento](https://msdn.microsoft.com/library/windows/apps/Bg124529) (e, exceto onde observado, para sua taxa de registro na conta de desenvolvedor).

| País/região | Reg individuais. taxa | Reg empresariais. taxa | Pagamento | PayPal |
|----------------|---------------------|------------------|--------|--------|
...
|  Bangladesh | 1468 BDT | 7600 BDT | BDT pago ao banco | Não | 
...
|  Egito | 133 EGP | 600 EGP | EGP pago ao banco | Não | 
...
|  Sri Lanka | 19 USD | 99 USD | LKR pago ao banco | Não | 
...
* [Abrindo uma conta de desenvolvedor](opening-a-developer-account.md)
* [Configurando sua conta de pagamento e formulários de imposto](setting-up-your-payout-account-and-tax-forms.md)
