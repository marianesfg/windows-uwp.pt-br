...
Uma configuração assim efeito os usuários podem personalizar é desativar efeitos de transparência ativar/desativar. Isso pode ser encontrado no aplicativo configurações em personalização > cores, ou por meio do aplicativo Configurações > facilidade de acesso > exibição.
...
Essa API fornece métodos e um ouvinte de eventos que pode ser usado para tornar o efeito dimensionamento decisões para a interface do usuário do aplicativo. O recurso detecta quão bem o sistema pode manipular complexa composição e renderização operações e, em seguida, retorna as informações em um modelo para os desenvolvedores utilizar fácil consumir.

### <a name="using-composition-capabilities"></a>Usando recursos de composição

A funcionalidade de CompositionCapabilities já está sendo usada para recursos como material acrílico, onde o material utilizará um efeito eficiente mais dependendo do cenário e hardware.
...
1. Registramos um ouvinte de eventos alterados recursos para seu aplicativo.
