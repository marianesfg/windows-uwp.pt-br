...
Por exemplo, considere um objeto comercial em que haja uma lista de "Times" (lista ordenada), cada uma tendo um dicionário de "Jogadores" onde cada jogador é citado pelo último nome. Um caminho de propriedade de exemplo para um player específico na segunda equipe é: "Teams\[1\]. Os players\[Smith\]". (Você usa 1 para indicar o segundo item em "Times" porque a lista é indexada com zero.)

**Observação**  o suporte à indexação de fontes de C++ dados é limitado; consulte a [ligação de dados em profundidade](https://docs.microsoft.com/windows/uwp/data-binding/data-binding-in-depth).
...
Para especificar um item filho em uma propriedade de coleção, você pode usar um indexador numérico. Use caracteres de colchetes "\[\]" em volta do valor de índice inteiro. Você pode fazer referência apenas a listas ordenadas, não a dicionários. Como uma coleção não é um valor que pode ser animado, o uso de um indexador nunca pode ser a propriedade final em um caminho de propriedade.
