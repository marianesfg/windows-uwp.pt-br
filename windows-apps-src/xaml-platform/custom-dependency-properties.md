...
> [!WARNING]
> Em circunstâncias excepcionais, as implementações wrapper devem executar somente o [ **GetValue** ](https://docs.microsoft.com/uwp/api/windows.ui.xaml.dependencyobject.getvalue) e [ **SetValue** ](https://docs.microsoft.com/uwp/api/windows.ui.xaml.dependencyobject.setvalue) operações. Caso contrário, você obterá um comportamento diferente quando a propriedade for definida via XAML, em comparação à definição via código. Por uma questão de eficiência, o analisador XAML ignora os wrappers ao definir as propriedades de dependência e se comunica com o repositório de backup por meio do **SetValue**.
...
> [!NOTE]
> Se você estiver definindo uma [ **CreateDefaultValueCallback** ](https://docs.microsoft.com/uwp/api/windows.ui.xaml.createdefaultvaluecallback) implementação, você deve usar o método de utilitário [ **PropertyMetadata.Create** ](https://docs.microsoft.com/uwp/api/windows.ui.xaml.propertymetadata.create)em vez de chamar um [ **PropertyMetadata** ](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.PropertyMetadata) construtor para definir o **PropertyMetadata** instância.
...
- As notificações de alterações de coleção são melhor tratadas por meio de interfaces, como **INotifyPropertyChanged** ou **INotifyCollectionChanged**, ou com a derivação do tipo de coleção de [**ObservableCollection&lt;T&gt;** ](https://docs.microsoft.com/dotnet/api/system.collections.objectmodel.observablecollection-1?redirectedfrom=MSDN).
