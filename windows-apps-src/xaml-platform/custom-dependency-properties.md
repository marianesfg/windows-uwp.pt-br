...
> [!NOTE]
> Se você estiver definindo uma propriedade anexada personalizada, você geralmente omitir o wrapper. Em vez disso, escreva um estilo diferente de acessador que possa ser usado pelo processador XAML. Veja [Propriedades anexadas personalizadas](custom-attached-properties.md). 
...
> [!WARNING]
> Embora o nome que você fornece que aqui pode ser qualquer cadeia de caracteres de identificador que seja válido na programação para a linguagem de sua escolha, você geralmente deseja ser capaz de definir sua propriedade de dependência no XAML muito. Para definir em XAML, o nome da propriedade deve ser um nome XAML válido. Para obter mais informações, consulte [Visão geral de XAML](xaml-overview.md).
...
> [!WARNING]
> Em circunstâncias excepcionais, as implementações wrapper devem executar somente o [ **GetValue** ](https://msdn.microsoft.com/library/windows/apps/br242359) e [ **SetValue** ](https://msdn.microsoft.com/library/windows/apps/br242361) operações. Caso contrário, você obterá um comportamento diferente quando a propriedade for definida via XAML, em comparação à definição via código. Por uma questão de eficiência, o analisador XAML ignora os wrappers ao definir as propriedades de dependência e se comunica com o repositório de backup por meio do **SetValue**.
...
> [!NOTE]
> Se você estiver definindo uma [ **CreateDefaultValueCallback** ](https://msdn.microsoft.com/library/windows/apps/hh701812) implementação, você deve usar o método de utilitário [ **PropertyMetadata.Create** ](https://msdn.microsoft.com/library/windows/apps/hh702099)em vez de chamar um [ **PropertyMetadata** ](https://msdn.microsoft.com/library/windows/apps/br208771) construtor para definir o **PropertyMetadata** instância.
...
> [!NOTE]
> Não registre com um valor padrão de [ **UnsetValue**](https://msdn.microsoft.com/library/windows/apps/br242371). Se fizer isso, os consumidores da propriedade poderão ficar confusos e haverá consequências indesejadas no sistema de propriedades.
