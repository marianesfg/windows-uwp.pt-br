...
Há dois valores possíveis para a regra de preenchimento opcionais: **F0** ou **F1**. (O **F** está sempre em maiusculas.) **F0** é o valor padrão; ela produz **EvenOdd** preencher o comportamento, portanto, normalmente você não especificá-lo. Use **F1** para obter o comportamento de preenchimento **Nonzero**. Esses valores de preenchimento se alinham aos valores da enumeração [**FillRule**](https://msdn.microsoft.com/library/windows/apps/br210030).
