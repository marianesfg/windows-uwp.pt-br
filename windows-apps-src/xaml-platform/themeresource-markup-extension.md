...
As regras pelas quais **ThemeResource** é resolvido para um item em um dicionário de recursos costumam ser as mesmas de **StaticResource**. Uma pesquisa de **ThemeResource** pode ser estendida para os arquivos do [**ResourceDictionary**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.ResourceDictionary) que são referenciados em uma coleção de [**ThemeDictionaries**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.resourcedictionary.themedictionaries), mas um **StaticResource** também pode fazer isso. A diferença é que um **ThemeResource** pode ser reavaliado no tempo de execução e um **StaticResource** não.
...
-   Pincéis, em determinadas cores para [**SolidColorBrush**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Media.SolidColorBrush). Eles compõem cerca de 80% dos usos de **ThemeResource** nos modelos de controle XAML padrão (generic.xaml).
-   Valores de pixel para bordas, deslocamentos, margens e preenchimento, etc.
-   Propriedades de fonte como **FontFamily** ou **FontSize**.
-   Modelos completos para um número limitado de controles que costumam ter o estilo do sistema e ser usados para apresentação dinâmica, como [**GridViewItem**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.GridViewItem) e [**ListViewItem**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.ListViewItem).
-   Estilos de exibição de texto (geralmente para mudar a cor da fonte, a tela de fundo e possivelmente o tamanho).

O Windows Runtime fornece um conjunto de recursos especialmente concebido para ser referenciado por **ThemeResource**. Todos esses recursos estão listados como parte do arquivo XAML themeresources.xaml, que está disponível na pasta include/winrt/xaml/design integrante do Software Development Kit do Windows (SDK do Windows). Para conhecer a documentação sobre pincéis de temas e estilos adicionais que estão definidos em themeresources.xaml, consulte [Recursos de temas XAML](https://docs.microsoft.com/windows/uwp/controls-and-patterns/xaml-theme-resources). Os pincéis estão documentados em uma tabela que informa o valor de cor de cada pincel em cada um dos três temas ativos possíveis.
...
O uso de **ThemeResource** pode ser visto em uma série de valores dependentes. Por exemplo, um valor [**Color**](https://docs.microsoft.com/uwp/api/Windows.UI.Color) usado por um [**SolidColorBrush**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Media.SolidColorBrush) que também é um recurso de chave pode usar uma referência **ThemeResource**. Mas quaisquer propriedades de interface do usuário que usem o recurso de chave **SolidColorBrush** também deverão usar uma referência **ThemeResource**, para que cada propriedade de tipo de [**Brush**](/uwp/api/Windows.UI.Xaml.Media.Brush) que esteja ativando um valor dinâmico mude quando o tema mudar.
...
Veja a seguir alguns exemplos de XAML extraídos dos arquivos padrão generic.xaml e themeresources.xaml para ilustrar como usar **ThemeResource**. Vamos analisar apenas um modelo (o [**Button**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Button) padrão) e como duas propriedades são declaradas ([**Background**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.control.background) e [**Foreground**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.control.foreground)) para serem responsivas às alterações de temas.
...
Aqui, as propriedades assumem um valor [**Brush**](/uwp/api/Windows.UI.Xaml.Media.Brush) e a referência aos recursos [**SolidColorBrush**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Media.SolidColorBrush) nomeadas `ButtonBackgroundThemeBrush` e `ButtonForegroundThemeBrush` são feitas usando **ThemeResource**.

Essas mesmas propriedades também são ajustadas por alguns dos estados visuais para um [**Button**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Button). Notavelmente, a cor da tela de fundo muda quando um botão é clicado. Aqui também, as animações de [**Background**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.control.background) e [**Foreground**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.control.foreground) no storyboard de estados visuais usam objetos [**DiscreteObjectKeyFrame**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Media.Animation.DiscreteObjectKeyFrame) e referências a pincéis com **ThemeResource** como o valor chave.
...
Microsoft Visual Studio 2013 pode incluir possíveis valores de chave nas listas suspensas Microsoft IntelliSense quando você usa o **{ThemeResource}** extensão de marcação em uma página XAML. Por exemplo, assim que você digita "{ThemeResource", todas as chaves de recurso dos [recursos de tema XAML](https://docs.microsoft.com/windows/uwp/controls-and-patterns/xaml-theme-resources) são exibidas.
...
* [Recursos de tema do XAML](https://docs.microsoft.com/windows/uwp/controls-and-patterns/xaml-theme-resources)
