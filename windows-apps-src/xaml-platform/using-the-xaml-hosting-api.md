...
A partir do SDK do Windows 10 Insider Preview build 17709, aplicativos da área de trabalho não UWP (incluindo aplicativos C++ Win32, WPF e Windows Forms) podem usar o *XAML da UWP que hospeda a API* para hospedar controles UWP em qualquer elemento de interface do usuário que está associado um (identificador de janela HWND). Essa API permite que os aplicativos da área de trabalho não UWP usar os recursos de interface do usuário do Windows 10 mais recentes que só estão disponíveis por meio de controles UWP. Por exemplo, aplicativos da área de trabalho não UWP podem usar essa API para hospedar controles UWP que usam o [Sistema de Design fluente](../design/fluent-design-system/index.md) e oferecer suporte à [Tinta do Windows](../design/input/pen-and-stylus-interactions.md).
...
O XAML da UWP que hospeda a API fornece a infraestrutura de baixo nível para hospedar controles UWP em aplicativos da área de trabalho. Alguns tipos de aplicativos da área de trabalho têm a opção de usar APIs alternativo, mais conveniente para realizar essa meta.  
...
* Para aplicativos WPF e Windows Forms, recomendamos que você use os [controles empacotados](xaml-host-controls.md#wrapped-controls) e [controles de host](xaml-host-controls.md#host-controls) no Kit de ferramentas de comunidade do Windows em vez do XAML da UWP que hospeda a API. Esses controles usam o XAML da UWP que hospeda a API internamente e proporcionar uma experiência de desenvolvimento mais simples. No entanto, você pode usar o XAML da UWP que hospeda API diretamente nesses tipos de aplicativos, se você escolher.

## <a name="related-samples"></a>Exemplos relacionados

A maneira como você usar o XAML da UWP que hospeda a API em seu código depende do seu tipo de aplicativo, o design do seu aplicativo e de outros fatores. Para ilustrar como usar essa API no contexto de um aplicativo completo, este artigo se refere ao código dos exemplos a seguir.

### <a name="c-win32"></a>C++ Win32

Há vários exemplos que demonstram como usar o XAML da UWP que hospeda a API em um aplicativo C++ Win32 no GitHub:
...
O XAML da UWP que hospeda a API inclui [**DesktopWindowXamlSource**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.hosting.desktopwindowxamlsource), [**WindowsXamlManager**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.hosting.windowsxamlmanager)e outros tipos relacionados no namespace [**Hosting**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.hosting) . Aplicativos da área de trabalho podem usar essa API para renderizar os controles UWP e navegação de foco do teclado de e para os elementos de rota. Aplicativos da área de trabalho também podem dimensionar e posicionar os controles UWP conforme desejado.
...
2. Criar um objeto [**DesktopWindowXamlSource**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.hosting.desktopwindowxamlsource) e anexá-lo a um elemento de interface do usuário pai em seu aplicativo que está associado um identificador de janela.
    ...
    1. Criar um objeto **DesktopWindowXamlSource** e convertê-lo para a interface **IDesktopWindowXamlSourceNative** COM. Essa interface é declarada no ```windows.ui.xaml.hosting.desktopwindowxamlsource.h``` arquivo de cabeçalho no SDK do Windows. Em um projeto C++ Win32, você pode fazer referência a esse arquivo de cabeçalho diretamente. Em um projeto do WPF ou Windows Forms, você precisará declarar essa interface no código do seu aplicativo usando o atributo [**ComImport**](https://docs.microsoft.com/dotnet/api/system.runtime.interopservices.comimportattribute) . Certifique-se de sua declaração de interface corresponde exatamente a declaração da interface no ```windows.ui.xaml.hosting.desktopwindowxamlsource.h```.
...
> [!IMPORTANT]
> Atualmente, controles personalizados de XAML da UWP de partes 3ª só têm suporte em aplicativos c# WPF e Windows Forms. Portanto, você pode compilar em relação a ele em seu aplicativo, você deve ter o código-fonte para os controles.
...
2. Em sua função [**WindowProc**](https://msdn.microsoft.com/library/windows/desktop/ms633573.aspx) , ouvir a mensagem [**WM_DPICHANGED**](https://docs.microsoft.com/windows/desktop/hidpi/wm-dpichanged) . Em resposta a esta mensagem:
    ...
    * Faça os ajustes necessários para a aparência e o layout do seu controle UWP. O exemplo de código a seguir ajusta o [**preenchimento**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.grid.padding) do controle **Windows.UI.Xaml.Controls.Grid** hospedado em resposta às alterações de DPI.
...
O XAML que hospeda a API compartilha as mesmas limitações que todos os outros tipos de controles de host XAML para Windows 10. Para obter uma lista detalhada, consulte as [limitações de controle de host XAML](xaml-host-controls.md#limitations).
