...
2. Um alerta de segurança será exibido no Windows Defender, selecione "permitir acesso". Quando VS Code abrir, você deverá ver o indicador de host de conexão remota, no canto inferior esquerdo, informando que você está editando em **WSL: Ubuntu-18.04 @ no__t-0.
...
4. Depois que a instalação for concluída, será necessário selecionar o botão azul **recarregar necessário** . Isso recarregará VS Code e exibirá um **WSL: UBUNTU-18, 4-instalação da seção @ no__t-0 na janela extensões de VS Code mostrando que você instalou a extensão do Python.
...
1. Abra a janela VS Code explorador de arquivos digitando **Ctrl + Shift + E** (ou use o menu para navegar até a **exibição** > **Explorer**).
...
2. Crie um diretório para seu projeto: `mkdir HelloWorld-Flask` e, em seguida, `cd HelloWorld-Flask` para inserir o diretório.
...
6. Ative o ambiente virtual que você criou na etapa #3 usando seu terminal Bash em VS Code: `source .venv/bin/activate`. Se funcionou, você deve ver (. venv) antes do prompt de comando.

7. Instale o Flask no ambiente virtual digitando: `python3 -m pip install flask`. Verifique se ele está instalado inserindo: `python3 -m flask --version`.
...
14. Abra o navegador da Web padrão na página renderizada, **Ctrl + clique** na URL http://127.0.0.1:5000/ no terminal. Você deverá ver a seguinte mensagem em seu navegador:
...
2. Crie um diretório para seu projeto: `mkdir HelloWorld-Django` e, em seguida, `cd HelloWorld-Django` para inserir o diretório.
...
8. Em seguida, execute o seguinte comando para criar o projeto Django:
    ...
    O comando `startproject` pressupõe (pelo uso de `.` no final) que a pasta atual é a pasta do projeto e cria o seguinte dentro dela:

    - `manage.py`: O utilitário administrativo da linha de comando Django para o projeto. Você executa comandos administrativos para o projeto usando `python manage.py <command> [options]`.

    - Uma subpasta denominada `web_project`, que contém os seguintes arquivos:
...
9. Para verificar o projeto Django, inicie o servidor de desenvolvimento do Django usando o comando `python3 manage.py runserver`. O servidor é executado na porta padrão 8000 e você deve ver uma saída semelhante à seguinte saída na janela do terminal:
    ...
    Quando você executa o servidor pela primeira vez, ele cria um banco de dados SQLite padrão no arquivo `db.sqlite3`, que é destinado a fins de desenvolvimento, mas pode ser usado na produção para aplicativos Web de baixo volume. Além disso, o servidor Web interno do Django é destinado *apenas* para fins de desenvolvimento local. No entanto, quando você implanta em um host da Web, o Django usa o servidor Web do host em vez disso. O módulo `wsgi.py` no projeto Django cuida da conexão com os servidores de produção.
...
10. `Ctrl+click` a URL `http://127.0.0.1:8000/` na janela saída do terminal para abrir o navegador padrão para esse endereço. Se o Django for instalado corretamente e o projeto for válido, você verá uma página padrão. A janela saída de terminal do VS Code também mostra o log do servidor.
...
12. Agora, para criar um aplicativo Django, execute o comando `startapp` do utilitário administrativo na pasta do projeto (onde `manage.py` reside):

    ```bash
    python3 manage.py startapp hello
    ```

    O comando cria uma pasta chamada `hello` que contém um número de arquivos de código e uma subpasta. Desses, você trabalha com frequência com `views.py` (que contém as funções que definem páginas em seu aplicativo Web) e `models.py` (que contém classes que definem seus objetos de dados). A pasta `migrations` é usada pelo utilitário administrativo do Django para gerenciar versões de banco de dados, conforme discutido posteriormente neste tutorial. Também há arquivos `apps.py` (configuração de aplicativo), `admin.py` (para criar uma interface administrativa) e `tests.py` (para testes), que não são abordados aqui.
...
14. Crie um arquivo, `hello/urls.py`, com o conteúdo abaixo. O arquivo `urls.py` é onde você especifica padrões para rotear URLs diferentes para suas exibições apropriadas. O código a seguir contém uma rota para mapear a URL raiz do aplicativo (`""`) para a função `views.home` que você acabou de adicionar a `hello/views.py`:
...
15. A pasta `web_project` também contém um arquivo `urls.py`, que é onde o roteamento de URL é realmente Tratado. Abra `web_project/urls.py` e modifique-o para corresponder ao código a seguir (você pode manter os comentários instrutivos, se desejar). Esse código efetua pull no `hello/urls.py` do aplicativo usando `django.urls.include`, que mantém as rotas do aplicativo contidas no aplicativo. Essa separação é útil quando um projeto contém vários aplicativos.
...
17. No VS Code terminal, execute o servidor de desenvolvimento com `python manage.py runserver` e abra um navegador para `http://127.0.0.1:8000/` para ver uma página que renderiza "Olá, Django".
